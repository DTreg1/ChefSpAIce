...
- **Authentication:** Custom username/password with SHA-256 hashing, social login (Apple Sign-In for iOS, Google Sign-In for Android), 30-day session tokens, guest mode by default. The OnboardingScreen now includes integrated sign-in/sign-up UI on the first step, combining app introduction with authentication options. New users see onboarding with embedded auth; returning users (who completed onboarding but signed out) go to a dedicated SignInScreen. Social auth uses `expo-apple-authentication` for native iOS Apple Sign-In and `expo-auth-session` with Google OAuth for cross-platform Google Sign-In. Backend routes at `/api/auth/social/apple` and `/api/auth/social/google` verify tokens and create/link accounts with `auth_providers` table. Apple token verification uses `apple-signin-auth` package with strict JWKS-based cryptographic signature validation (requires `APPLE_CLIENT_ID` environment variable).
...
- **Data Export:** Export inventory and recipes as CSV or PDF files. Uses expo-print for PDF generation with styled layouts and expo-sharing for cross-platform file sharing. Export buttons available on InventoryScreen (filter row), RecipesScreen (filter row), and RecipeDetailScreen (header). Web platform uses download links and print dialogs. Export utility library at `client/lib/export.ts`.
- **Advanced Inventory Filtering:** Multi-criteria filtering with expiration status (expired/expiring/fresh), storage location (fridge/freezer/pantry/etc.), food group (grains/vegetables/fruits/protein/dairy), and nutrition filters. Sort options include expiration date, name A-Z, quantity, and recently added. Filter summary shows matching item count with one-tap "Clear" to reset all filters. Enhanced search also matches storage locations.
