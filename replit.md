...
- **Cloud Sync:** All authenticated user data is synced to PostgreSQL as JSON via `/api/auth/sync` endpoint, managed by a real-time sync manager with retry logic and conflict resolution. **Preference Sync:** User preferences from onboarding/settings are validated with Zod schema (`syncPreferencesSchema`) and mapped to `users` table columns: `servingSize→householdSize`, `dietaryRestrictions`, `cuisinePreferences→favoriteCategories`, `storageAreas→storageAreasEnabled`, `cookingLevel→cookingSkillLevel` (basic→beginner, professional→advanced), `expirationAlertDays`. Sync response includes `prefsSynced` boolean and optional `prefsError` for graded validation feedback. **Battery Optimizations:**
