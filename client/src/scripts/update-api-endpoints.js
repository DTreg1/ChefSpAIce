...
function updateFile(filePath) {
  ...
  for (const [oldEndpoint, newEndpoint] of Object.entries(endpointMappings)) {
    ...
    const patterns = [
      // Match in strings
      new RegExp(`(['"\`])${oldEndpoint.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(/|['"\`])`, 'g'),
      new RegExp(`(['"\`])${oldEndpoint.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'gm'),
    ];
    ...
    for (const pattern of patterns) {
      ...
      if (matches) {
        content = content.replace(pattern, (match, quote1, quote2) => {
          if (quote2 && quote2 !== '/' && (quote2 === '"' || quote2 === "'" || quote2 === '`')) {
            return `${quote1}${newEndpoint}${quote2}`;
          }
          return `${quote1}${newEndpoint}${quote2 || ''}`;
        });
        modified = true;
        changes.push(`  ${oldEndpoint} → ${newEndpoint} (${matches.length} occurrences)`);
      }
    }
  }
...
console.log('📝 Note: The backend automatically redirects old endpoints, so the app will continue to work during migration.');