...
export default function Nutrition() {
  ...
  const { data: missingItems, isLoading: missingLoading } = useQuery<
    MissingNutritionItem[]
  >({
  ...
  const refreshNutritionMutation = useMutation({
    mutationFn: async (itemId: string) => {
      return await apiRequest(
        `/api/food-items/${itemId}/refresh-nutrition`,
        "POST",
        {},
      );
    },
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Could not fetch nutrition data for this item",
        variant: "destructive",
      });
    },
  });
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              {macroData.map((macro) => (
                <Card
                  key={macro.name}
                  data-testid={`card-${macro.name.toLowerCase()}`}
                >
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-medium flex items-center gap-2">
                  ...
                  <CardContent>
                    <div
                      className="text-2xl font-bold"
                      data-testid={`text-${macro.name.toLowerCase()}`}
                    >
                      {macro.value}g
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
            ...
            >
              <div className="space-y-4">
                <div
                  className="flex items-center w-full h-8 rounded-lg overflow-hidden"
                  data-testid="macro-stacked-bar"
                >
                  ...
                  {macroData.map((macro) => {
                    ...
                    return (
                      <div
                        ...
                        data-testid={`macro-legend-${macro.name.toLowerCase()}`}
                      >
                        <div
                          className={`w-3 h-3 rounded-full ${macro.color}`}
                        />
            ...
            >
              {itemsLoading ? (
                ...
                <p className="text-muted-foreground text-center py-4">
                  No items with nutrition data
                </p>
              ) : (
                <div className="space-y-3">
                  {items.map((item) => {
                    const servingSize =
                      parseFloat(item.nutrition?.servingSize || "100") || 100;
                    const multiplier = item.weightInGrams / servingSize;
                    const protein = (item.nutrition?.protein || 0) * multiplier;
                    const carbs =
                      (item.nutrition?.carbohydrates || 0) * multiplier;
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <Badge variant="secondary">
                            ...
                            {Math.round(
                              (item.nutrition?.calories || 0) * multiplier,
                            )}{" "}
                            kcal
                          </Badge>
                        </div>
                        ...
                        {itemTotalMacros > 0 && (
                          <div className="space-y-2">
                            <div
                              className="flex items-center w-full h-6 rounded overflow-hidden"
                              data-testid={`item-macro-bar-${item.id}`}
                            >
                              {protein > 0 && (
                                <div
                                  className="h-full bg-red-500 flex items-center justify-center"
                                  style={{
                                    width: `${(protein / itemTotalMacros) * 100}%`,
                                  }}
                                >
                              ...
                              {carbs > 0 && (
                                <div
                                  ...
                                  style={{
                                    width: `${(carbs / itemTotalMacros) * 100}%`,
                                  }}
                                >
                ...
                {missingLoading ? (
                  ...
                  <>
                    <Alert className="mb-4">
                      ...
                      <AlertDescription>
                        Click "Refresh Nutrition" to automatically fetch
                      ...
                      {missingItems.map((item) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              refreshNutritionMutation.mutate(item.id)
                            }
                            disabled={refreshNutritionMutation.isPending}
                            data-testid={`button-refresh-${item.id}`}
                          >
                            <RefreshCw
                              className={`w-4 h-4 mr-2 ${refreshNutritionMutation.isPending ? "animate-spin" : ""}`}
                            />
                            Refresh Nutrition
                          </Button>
                        </div>
