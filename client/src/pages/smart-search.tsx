...
import { Search, Sparkles, Tag, FolderOpen, Loader2, Brain, Utensils, Package } from "lucide-react";
...
export default function SmartSearch() {
  ...
  const [selectedContentType, setSelectedContentType] = useState<'recipe' | 'inventory' | null>(null);
  ...
  const renderContentCard = (item: any) => {
    ...
    return (
      ...
      >
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              {isRecipe ? <Utensils className="h-4 w-4" /> : <Package className="h-4 w-4" />}
              {content?.name || content?.title || 'Unnamed'}
            </CardTitle>
            {item.similarity !== undefined && (
              <Badge variant="secondary" data-testid={`badge-similarity-${item.contentId}`}>
                {Math.round(item.similarity * 100)}% match
              </Badge>
            )}
            {item.score !== undefined && (
              <Badge variant="secondary" data-testid={`badge-score-${item.contentId}`}>
        ...
        <CardContent>
          <CardDescription>
            {content?.description || content?.notes || 'No description available'}
          </CardDescription>
          <div className="mt-3 flex flex-wrap gap-1">
            {item.category && (
              <Badge variant="outline" className="text-xs" data-testid={`badge-category-${item.contentId}`}>
            ...
            {!isRecipe && content?.expirationDate && (
              <Badge variant="outline" className="text-xs" data-testid={`badge-expiry-${item.contentId}`}>
                Expires: {new Date(content.expirationDate).toLocaleDateString()}
              </Badge>
            )}
            {isRecipe && content?.prepTime && (
              <Badge variant="outline" className="text-xs" data-testid={`badge-prep-${item.contentId}`}>
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Try: "healthy dinner recipes", "what expires soon?", "ingredients for pasta", or "show me vegetarian options"
        </p>
      </div>
      ...
      <Tabs defaultValue="search" className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="natural" data-testid="tab-natural">Natural Query Results</TabsTrigger>
          <TabsTrigger value="related" disabled={!selectedContentId} data-testid="tab-related">
            Related Content
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="search" className="mt-4">
          {searchMutation.data && (
            <div className="space-y-4">
              ...
              {searchMutation.data.results?.map((item: any) => renderContentCard(item))}
            </div>
          )}
        ...
        <TabsContent value="natural" className="mt-4">
          {nlQueryMutation.data && (
            <div className="space-y-4">
              ...
              {nlQueryMutation.data.results?.map((item: any) => renderContentCard({
                ...item,
