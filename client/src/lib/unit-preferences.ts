  ...
  try {
    ...
    if (stored) {
      ...
      if (prefs.length > MAX_PREFERENCES) {
        prefs.sort(
          (a: UnitPreference, b: UnitPreference) => b.lastUsed - a.lastUsed,
        );
...
export function getUserUnitPreference(
  ...
  for (const pattern of patterns) {
    ...
    if (exactMatch) {
      const confidence =
        exactMatch.count >= CONFIDENCE_THRESHOLD
          ? "high"
          : exactMatch.count >= 2
            ? "medium"
            : "low";
  ...
  for (const pref of preferences) {
    if (
      pref.foodPattern.length > PATTERN_MIN_LENGTH &&
      normalized.includes(pref.foodPattern)
    ) {
...
export function getPreferenceStats(): {
  ...
  const highConfidenceCount = preferences.filter(
    (p) => p.count >= CONFIDENCE_THRESHOLD,
  ).length;

  const topPatterns = preferences.slice(0, 10).map((p) => ({
