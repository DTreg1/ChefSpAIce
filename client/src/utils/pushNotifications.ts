...
class PushNotificationService {
  ...
  private permissionState: "granted" | "denied" | "prompt" | "unknown" =
    "unknown";
  ...
  private async initializeWebPush(): Promise<void> {
    try {
      ...
      this.permissionState = permission as
        | "prompt"
        | "unknown"
        | "granted"
        | "denied";
  ...
  private async sendWebSubscriptionToBackend(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || "Failed to send web subscription to backend",
        );
      }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          ...
          .catch(() => ({ error: "Unknown error" }));
        throw new Error(
          errorData.error || "Failed to send push token to backend",
        );
      }
