...
export function TrendPredictor({ currentTrends, historicalData }: TrendPredictorProps) {
  ...
  const generatePredictions = () => {
    const predictions = currentTrends.map(trend => {
      ...
      return {
        trend,
        prediction: {
          nextWeekGrowth: Math.round((value - currentValue) / currentValue * 100),
          peakDay: futurePoints.reduce((max, p, i) => p.predicted > futurePoints[max].predicted ? i : max, 0) + 1,
  ...
  const chartData = predictions.length > 0 
    ? predictions[0].prediction.futurePoints.map((point, index) => ({
        ...
        ...predictions.reduce((acc, p) => ({
          ...acc,
          [p.trend.trendName]: p.prediction.futurePoints[index]?.predicted || 0,
        }), {}),
      }))
    : [];
  ...
  const opportunities = predictions
    .filter(p => p.prediction.nextWeekGrowth > 50 && p.prediction.confidence > 0.6)
    .map(p => ({
            ...
            <div className="space-y-3">
              {opportunities.map((opp, index) => (
                <Alert key={index} className="hover-elevate">
                  ...
                  <AlertDescription>
                    <p>{opp.description}</p>
                    <p className="font-medium mt-2">Recommended: {opp.action}</p>
                  </AlertDescription>
                </Alert>
              ))}
            </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {risks.map((risk, index) => (
                <Alert key={index} variant="destructive" className="hover-elevate">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-lg">
                    {prediction.nextWeekGrowth > 0 ? "+" : ""}{prediction.nextWeekGrowth}%
                  </span>
                </div>
                
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">Peak Expected</span>
