...
export function RevenueImpact({
  ...
  const projectionData = useMemo(() => {
    ...
    for (let i = 0; i <= days; i++) {
      ...
      const projectedRevenue =
        baseRevenue * (1 + (revenueImpact / 100) * adoptionRate);
      const optimisticRevenue =
        baseRevenue * (1 + (revenueImpact / 100) * 1.2 * adoptionRate);
      const pessimisticRevenue =
        baseRevenue * (1 + (revenueImpact / 100) * 0.8 * adoptionRate);
  ...
  const totalProjectedRevenue = projectionData.reduce(
  ...
  const funnelData = [
    {
      stage: "Views",
      current: 1000,
      projected: 1000 * (1 + demandImpact / 100),
    },
    {
      stage: "Clicks",
      current: 200,
      projected: 200 * (1 + (demandImpact / 100) * 0.8),
    },
    {
      stage: "Cart Adds",
      current: 100,
      projected: 100 * (1 + (demandImpact / 100) * 0.6),
    },
    {
      stage: "Purchases",
      current: 50,
      projected: 50 * (1 + conversionImpact / 100),
    },
  ];
    ...
    <div className="space-y-6" data-testid="revenue-impact">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <p
                  className={`text-xl font-bold ${revenueImpact > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
                  {revenueImpact > 0 ? "+" : ""}
                  {projectedImpact.revenue}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p
                  className={`text-xl font-bold ${conversionImpact > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
                  {conversionImpact > 0 ? "+" : ""}
                  {projectedImpact.conversionRate}
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p
                  className={`text-xl font-bold ${demandImpact > 0 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                >
                  {demandImpact > 0 ? "+" : ""}
                  {projectedImpact.demandChange}
                </p>
              </div>
            </div>
                  ...
                  return (
                    <div className="bg-background border rounded-lg p-2 shadow-lg">
                      ...
                      <p className="text-sm text-gray-600">
                        Current: ${data.current.toFixed(0)}
                      </p>
                      <p className="text-sm text-blue-600">
                        Projected: ${data.projected.toFixed(0)}
                      </p>
                      <p className="text-sm text-green-600">
                        Optimistic: ${data.optimistic.toFixed(0)}
                      </p>
                      <p className="text-sm text-yellow-600">
                        Pessimistic: ${data.pessimistic.toFixed(0)}
                      </p>
                    </div>
                  );
                }}
      ...
      {impactSeverity === "positive" && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Strong Positive Impact Expected:</strong> This price
            optimization shows significant revenue potential with{" "}
            {projectedImpact.revenue} increase projected.
          </AlertDescription>
        </Alert>
      )}
      {impactSeverity === "negative" && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <strong>Negative Impact Warning:</strong> This price change may
            reduce revenue by {projectedImpact.revenue}. Consider alternative
            strategies.
          </AlertDescription>
        </Alert>
      )}
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={
                  Math.abs(demandImpact) > 20 ? "destructive" : "secondary"
                }
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <Badge
                variant={revenueImpact < -10 ? "destructive" : "secondary"}
              >
                {revenueImpact < -10
                  ? "High"
                  : revenueImpact < 0
