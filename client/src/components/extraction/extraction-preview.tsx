...
export function ExtractionPreview({
  ...
  const highlightedText = useMemo(() => {
    if (!highlightedField || !extractedFields[highlightedField]) return inputText;
  ...
  const getValidationBadge = () => {
    switch (validationStatus) {
      case 'validated':
        return <Badge variant="default" className="bg-green-600"><Check className="w-3 h-3 mr-1" />Validated</Badge>;
      case 'corrected':
        return <Badge variant="default" className="bg-blue-600"><Check className="w-3 h-3 mr-1" />Corrected</Badge>;
      case 'rejected':
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className={cn("font-semibold", getConfidenceColor(overallConfidence))}>
      ...
      <CardContent>
        <Tabs defaultValue="extracted" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="extracted" data-testid="tab-extracted">Extracted Data</TabsTrigger>
            <TabsTrigger value="original" data-testid="tab-original">Original Text</TabsTrigger>
          </TabsList>
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isHighlighted ? "border-primary bg-primary/5" : "hover:border-muted-foreground/50"
                    )}
                    onClick={() => setHighlightedField(isHighlighted ? null : fieldName)}
          ...
          <TabsContent value="original">
            ...
            {highlightedField && (
              <div className="mt-2 text-xs text-muted-foreground">
                Highlighting: <span className="font-medium">{highlightedField}</span>
              </div>
            )}
          </TabsContent>
        </Tabs>
