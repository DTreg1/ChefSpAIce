...
export function DowntimePredictor() {
  ...
  const downtimeByComponent =
    schedule?.predictions?.reduce(
      (acc, pred) => {
  ...
  const pieData = Object.entries(downtimeByComponent).map(
    ([component, downtime]) => ({
      name: component,
      value: downtime,
    }),
  ...
  const calculateRiskScore = () => {
    ...
    const criticalCount = schedule.predictions.filter(
      (p) => p.urgencyLevel === "critical",
    ).length;
    const highCount = schedule.predictions.filter(
      (p) => p.urgencyLevel === "high",
    ).length;
    const totalDowntime = schedule.estimatedDowntimeHours;

    const riskScore = Math.min(
      100,
      criticalCount * 30 + highCount * 15 + totalDowntime * 2,
    );
  ...
  const availability = (
    ((totalMinutesInMonth - estimatedDowntimeMinutes) / totalMinutesInMonth) *
    100
  ).toFixed(2);
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="text-center">
              ...
              <Badge
                className="mt-2"
                variant={riskScore > 70 ? "destructive" : "outline"}
              >
                {riskLevel} Risk
              </Badge>
            </div>
            ...
            <div className="text-center">
              ...
              <div className="text-3xl font-bold">
                {schedule?.estimatedDowntimeHours || 0}h
              </div>
          ...
          {riskScore > 70 && (
            <Alert className="mt-6 border-red-200">
              ...
              <AlertDescription>
                High downtime risk detected. Consider rescheduling non-critical
                maintenance or allocating additional resources to minimize
                impact.
              </AlertDescription>
            </Alert>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Distribution of predicted downtime
            </CardDescription>
          </CardHeader>
          <CardContent>
            {pieData.length > 0 ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Expected downtime over the next 4 weeks
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              ...
              <>
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      Combine{" "}
                      {
                        schedule.predictions.filter(
                          (p) => p.urgencyLevel === "low",
                        ).length
                      }{" "}
                      low-priority maintenance tasks into a single window to
                      reduce overall disruption
                    </div>
                  </div>
                </div>

                {schedule.predictions.some(
                  (p) => p.urgencyLevel === "critical",
                ) && (
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">
                        ...
                        critical issues immediately to prevent cascading
                        failures
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      Schedule maintenance during weekend early morning hours
                      (3-6 AM) for minimal user impact
                    </div>
                  </div>
                </div>
              </>
