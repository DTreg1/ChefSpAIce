...
export function UpgradeBadge({
  ...
  const getBadgeText = () => {
    if (type === "limit" && remaining !== undefined && max !== undefined) {
  ...
  const getBadgeColor = () => {
    if (type === "limit" && remaining !== undefined) {
      if (remaining === 0) return AppColors.error;
      if (remaining <= 2) return AppColors.warning;
    }
  ...
  if (compact) {
    return (
      <AnimatedPressable
        ...
        style={[
          styles.compactBadge,
          { backgroundColor: `${getBadgeColor()}20`, borderColor: getBadgeColor() },
          animatedStyle,
        ]}
        data-testid="badge-upgrade-compact"
      >
        <Ionicons name="lock-closed" size={12} color={getBadgeColor()} />
      </AnimatedPressable>
    );
  }
