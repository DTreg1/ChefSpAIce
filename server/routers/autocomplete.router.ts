...
router.post("/learn", isAuthenticated, async (req: any, res) => {
  try {
    ...
    await storage.recordFormInput(userId, fieldName, value, context);
...
router.post("/feedback", isAuthenticated, async (req: any, res) => {
  try {
    ...
    // Record the feedback
    const feedback = await storage.recordCompletionFeedback(feedbackData);
...
router.get("/history", isAuthenticated, async (req: any, res) => {
  try {
    ...
    const history = await storage.getUserFormHistory(
      userId,
...
router.delete("/history", isAuthenticated, async (req: any, res) => {
  try {
    ...
    await storage.clearUserFormHistory(userId);
