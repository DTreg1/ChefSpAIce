...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const retention = await storage.admin.experiments.calculateCohortRetention(
      id,
      [1, 7, 30],
    );
...
router.post(
  "/compare",
  asyncHandler(async (req, res) => {
    const { cohortIds, metrics = ["retention_day_7", "engagement_score"] } =
      req.body;
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const allCohorts = await storage.admin.experiments.getCohorts({
      isActive: true,
    });
    ...
    try {
      ...
      if (retention.retention.length > 1) {
        const day7Rate =
          retention.retention.find((r) => r.period === 7)?.rate || 0;
        const day30Rate =
          retention.retention.find((r) => r.period === 30)?.rate || 0;
        ...
        if (day7Rate < 30) {
          insights.push({
            ...
            actionRecommended:
              "Implement immediate re-engagement campaigns and onboarding improvements",
            confidenceScore: 0.9,
            supportingData: {
      ...
      const savedInsights = await Promise.all(
        insights.map((insight) =>
          storage.admin.experiments.createCohortInsight(
            insertCohortInsightSchema.parse(insight),
          ),
        ),
      );
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const insight = await storage.admin.experiments.updateCohortInsightStatus(
      insightId,
      status,
    );
