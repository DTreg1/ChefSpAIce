...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      const result =
        await storage.admin.experiments.upsertAbTestResult(validated);
      res.json(result);
    } catch (error: any) {
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const significance =
        await storage.admin.experiments.calculateStatisticalSignificance(
          testId,
        );
      ...
      if (process.env.OPENAI_API_KEY) {
        ...
        try {
          ...
          const metaA = (aggregated.variantA.metadata || {}) as Record<
            string,
            any
          >;
          const metaB = (aggregated.variantB.metadata || {}) as Record<
            string,
            any
          >;
          const configVariants = ((test.configuration as any)?.variants ||
        ...
        `;
          ...
          const sampleSize =
            (metaA.totalExposures || 0) + (metaB.totalExposures || 0);
          ...
          aiInsights =
            await storage.admin.experiments.upsertAbTestVariantMetric({
              testId,
              variant,
              sampleSize,
              conversionRate,
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const updated = await storage.admin.experiments.updateAbTest(
        id,
        req.body,
      );
      res.json(updated);
