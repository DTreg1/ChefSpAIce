...
router.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      const rule =
        await storage.admin.pricing.getPricingRuleByProduct(productId);

      if (!rule) {
        return res
          .status(404)
          .json({ error: "No pricing rule found for product" });
      }

      // Get current market conditions
      const demand = await storage.admin.pricing.getCurrentDemand(productId);
      const inventory =
        await storage.admin.pricing.getCurrentInventory(productId);
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a pricing optimization expert. Analyze market conditions and provide strategic pricing recommendations.",
              },
              {
                role: "user",
      ...
      const optimization = await storage.admin.pricing.calculateOptimalPrice(
        productId,
        {
          includeCompetition,
          targetRevenue: req.query.targetRevenue
            ? parseFloat(req.query.targetRevenue)
            : undefined,
          targetConversion: req.query.targetConversion
            ? parseFloat(req.query.targetConversion)
            : undefined,
        },
      );
      ...
      if (predictedDemand > demand.demandScore * 1.2) {
        ...
        optimization.reasoning.push(
          `Demand predicted to increase to ${predictedDemand.toFixed(0)}/100 - added 3% premium`,
        );
        ...
        optimization.reasoning.push(
          `Demand predicted to decrease to ${predictedDemand.toFixed(0)}/100 - applied 3% discount`,
        );
      }

      // Ensure price stays within bounds
      optimization.recommendedPrice = Math.max(
        rule.minPrice,
        Math.min(rule.maxPrice, optimization.recommendedPrice),
      );
      ...
      res.json({
        ...
        priceChange:
          (
            ((optimization.recommendedPrice - rule.basePrice) /
              rule.basePrice) *
            100
          ).toFixed(2) + "%",
        ...
        projectedImpact: {
          revenue:
            (optimization.projectedImpact.revenue * 100).toFixed(2) + "%",
          conversionRate:
            (optimization.projectedImpact.conversionRate * 100).toFixed(2) +
            "%",
          demandChange:
            (optimization.projectedImpact.demandChange * 100).toFixed(2) + "%",
        },
        marketConditions: {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to optimize price" });
    }
  }),
);
    ...
    try {
      ...
      for (const scenario of scenarios) {
        ...
        results.push({
          ...
          profitMargin:
            (
              ((scenario.price - rule.minPrice * 0.6) / scenario.price) *
              100
            ).toFixed(2) + "%",
        });
      }
      ...
      res
        .status(400)
        .json({ error: error.message || "Invalid simulation data" });
    }
  }),
);
...
router.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      const competitors =
        await storage.admin.pricing.getCompetitorPricing(productId);
      const rule =
        await storage.admin.pricing.getPricingRuleByProduct(productId);
      ...
      const pricePosition =
        rule.basePrice > avgCompetitorPrice * 1.05
          ? "above_market"
          : rule.basePrice < avgCompetitorPrice * 0.95
            ? "below_market"
            : "at_market";
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a competitive pricing analyst. Provide strategic insights on competitive positioning.",
              },
              {
                role: "user",
      ...
      res.json({
        ...
        priceGap:
          (
            ((rule.basePrice - avgCompetitorPrice) / avgCompetitorPrice) *
            100
          ).toFixed(2) + "%",
        competitors: competitors.map((c) => ({
          ...c,
          priceDifference:
            (((c.price - rule.basePrice) / rule.basePrice) * 100).toFixed(2) +
            "%",
        })),
        recommendation:
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to analyze competition" });
    }
  }),
);
...
router.put(
  ...
  asyncHandler(async (req: any, res: any) => {
    try {
      ...
      const existingRule = await storage.admin.pricing.getPricingRuleByProduct(
        ruleData.productId,
      );
      ...
      if (existingRule) {
        ...
        result = await storage.admin.pricing.updatePricingRule(
          existingRule.id,
          ruleData,
        );
      } else {
      ...
      res
        .status(400)
        .json({ error: error.message || "Invalid pricing rule data" });
    }
  }),
);
...
router.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      // Get overall metrics
      const metrics = await storage.admin.pricing.getPricingMetrics({
        startDate,
        endDate,
      });
      ...
      const productReports = await Promise.all(
        activeRules.map(async (rule) => {
          const performance = await storage.admin.pricing.getPricingPerformance(
            rule.productId,
            { startDate, endDate },
          );
          const history = await storage.admin.pricing.getPriceHistory(
            rule.productId,
            { startDate, endDate, limit: 10 },
          );
          const demand = await storage.admin.pricing.getCurrentDemand(
            rule.productId,
          );
          const inventory = await storage.admin.pricing.getCurrentInventory(
            rule.productId,
          );
          ...
          const totalUnits = performance.reduce(
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a pricing strategy advisor. Analyze pricing performance and provide actionable insights.",
              },
              {
                role: "user",
                ...
                Top Performers: ${metrics.topPerformingProducts
                  .slice(0, 3)
                  .map(
                    (p) =>
      ...
      res.json({
        ...
        summary: {
          ...
          averageConversionRate:
            (metrics.averageConversionRate * 100).toFixed(2) + "%",
          averagePriceChange:
            (metrics.averagePriceChange * 100).toFixed(2) + "%",
        ...
        recommendations: [
          metrics.averagePriceChange > 0.1
            ? "High price volatility detected - consider stabilizing prices"
            : null,
          metrics.averageConversionRate < 0.02
            ? "Low conversion rates - review pricing strategy"
            : null,
          productReports.some((p) => p.inventoryScore > 80)
            ? "High inventory on some products - consider promotional pricing"
            : null,
          productReports.some(
            (p) =>
              p.demandTrend === "increasing" &&
              Number(p.currentPrice) === Number(p.averagePrice),
          )
            ? "Opportunity to increase prices on trending products"
            : null,
        ].filter(Boolean),
      });
    } catch (error: any) {
...
router.post(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      ...
      await storage.admin.pricing.updatePricingRule(rule.id, {
        basePrice: price,
      });
      ...
      res.json({
        ...
        newPrice: price,
        priceChange:
          (((price - rule.basePrice) / rule.basePrice) * 100).toFixed(2) + "%",
...
router.get(
  ...
  asyncHandler(async (req: any, res: any) => {
    ...
    try {
      const history = await storage.admin.pricing.getPriceHistory(productId, {
        startDate,
        endDate,
        limit,
      });
      ...
      res.json({
        ...
        history: history.map((h) => ({
          ...
          priceChange: h.previousPrice
            ? (((h.price - h.previousPrice) / h.previousPrice) * 100).toFixed(
                2,
              ) + "%"
            : null,
        })),
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get price history" });
    }
  }),
);
