...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const predictions =
        await storage.platform.system.getMaintenancePredictions(
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Run analysis
      const predictions =
        await predictiveMaintenanceService.analyzeComponent(component);

      // Get recent metrics for health calculation
      const recentMetrics =
        await storage.platform.system.getSystemMetrics(component);
      ...
      res.json({
        ...
        analysis: {
          ...
          health: {
            ...
            avgAnomalyScore,
            recentAnomalies: recentMetrics.filter(
              (m: any) => ((m.metadata as any)?.anomalyScore || 0) > 0.5,
            ).length,
            status:
              avgAnomalyScore > 0.7
                ? "critical"
...
router.get(
  ...
  async (req, res, next) => {
    try {
      const schedule =
        await predictiveMaintenanceService.getMaintenanceSchedule();

      // Group by date
      const byDate = schedule.reduce(
        (acc, pred) => {
          const date = pred.predictedDate
            ? new Date(pred.predictedDate).toISOString().split("T")[0]
            : "unscheduled";
          if (!acc[date]) {
            acc[date] = [];
          }
      ...
      const totalDowntime = schedule.reduce(
        (sum, p) => sum + ((p.metadata as any)?.estimatedDowntime || 0),
        0,
      );
  ...
  async (req, res, next) => {
    try {
      ...
      res.json({
        ...
        history,
        predictionAccuracy:
          prediction?.recommendation === data.issue ? "accurate" : "inaccurate",
      });
    } catch (error) {
      next(error);
    }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const predictions =
        await storage.platform.system.getMaintenancePredictions("active");
  ...
  async (req, res, next) => {
    try {
      ...
      const stats = {
        ...
        anomalies: metrics.filter(
          (m) => ((m.metadata as any)?.anomalyScore || 0) > 0.5,
        ).length,
      };
      ...
      const stats = {
        ...
        successRate:
          (history.filter((h) => h.result === "success").length /
            history.length) *
          100,
        componentsServiced: Array.from(
          new Set(history.map((h) => h.component)),
        ),
      };
    ...
    try {
      const components = await Promise.all(
        Object.values(MONITORED_COMPONENTS).map(async (component) => {
          const health =
            await storage.platform.system.getComponentHealth(component);
          const predictions =
            await storage.platform.system.getMaintenancePredictions(
              "active",
              component,
          ...
          return {
            ...
            status:
              health.avgAnomalyScore > 0.7
                ? "critical"
                : health.avgAnomalyScore > 0.5
                  ? "warning"
                  : "healthy",
            ...
            metrics: {
              ...
              anomalies: health.recentMetrics.filter(
                (m: any) => ((m.metadata as any)?.anomalyScore || 0) > 0.5,
              ).length,
            },
          };
