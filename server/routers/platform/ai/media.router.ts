...
} from "../../../services/alt-text-generator.service";
...
function determineEditType(
  editReason?: string,
): "spelling" | "punctuation" | "speaker" | "content" | "other" {
  if (!editReason) return "content";

  const lowerReason = editReason.toLowerCase();
  if (lowerReason.includes("spell") || lowerReason.includes("typo"))
    return "spelling";
  if (
    lowerReason.includes("punctuat") ||
    lowerReason.includes("comma") ||
    lowerReason.includes("period")
  )
    return "punctuation";
  if (lowerReason.includes("speaker") || lowerReason.includes("attribution"))
    return "speaker";
  if (
    lowerReason.includes("content") ||
    lowerReason.includes("meaning") ||
      ...
      switch (filter.type) {
        ...
        case "brightness":
          image = image.modulate({
            brightness: 1 + (filter.intensity || 0) / 100,
          });
          break;
        case "contrast":
        ...
        case "saturation":
          image = image.modulate({
            saturation: 1 + (filter.intensity || 0) / 100,
          });
          break;
      }
    }
...
router.post(
  ...
  async (req: any, res: any) => {
    ...
    try {
      ...
      const { buffer, metadata } = await processWithSharp(
        inputPath,
        operations,
      );
      ...
      const outputPath = inputPath.replace(
        /\.[^.]+$/,
        `-enhanced.${operations.format || "jpg"}`,
      );
      ...
      res.json({
        ...
        compressionRatio:
          ((1 - buffer.length / req.file.size) * 100).toFixed(1) + "%",
        metadata: {
          width: metadata.width,
          height: metadata.height,
          format: metadata.format,
    ...
    try {
      ...
      if (!apiKey) {
        return res.status(500).json({
          error:
            "Remove.bg API key not configured. Please set REMOVEBG_API_KEY environment variable.",
        });
      }
...
router.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to process batch" });
    }
  },
);
      ...
      const defaultPresets = [
        {
          ...
          settings: {
            description:
              "Clean white background, enhanced colors, sharp details",
...
router.post("/images/presets", isAuthenticated, async (req: any, res: any) => {
  try {
    ...
    if (!name || !operations) {
      return res
        .status(400)
        .json({ error: "Name and operations are required" });
    }
      ...
      const ocrResult = await storage.platform.ai.createOcrResult(userId, {
        ...
        structuredData: {
          blocks: extractionResult.boundingBoxes?.map((box: any) => ({
            ...
            bbox: [box.x, box.y, box.width, box.height] as [
              number,
              number,
              number,
              number,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const detectionResult = await faceDetectionService.detectFaces(
        req.file.buffer,
      );
      ...
      const detectionRecord = await storage.platform.ai.createFaceDetection(
        userId,
        {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to detect faces" });
    }
  },
);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get privacy settings" });
    }
  },
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update privacy settings" });
    }
  },
);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      let imageMetadata = await storage.platform.ai.getImageMetadataByUrl(
        userId,
        imageUrl,
      );
      ...
      if (saveToDb) {
        if (!imageMetadata) {
          imageMetadata = await storage.platform.ai.createImageMetadata(
            userId,
            {
              url: imageUrl,
          ...
          imageMetadata = await storage.platform.ai.updateImageMetadata(
            userId,
            imageMetadata.id,
            {
              autoGeneratedAlt: result.altText,
        ...
        await storage.platform.ai.upsertAltTextQuality(
          imageMetadata.id,
          quality,
        );
      }
    ...
    try {
      ...
      for (const image of images) {
        try {
          ...
          const imageMetadata = await storage.platform.ai.createImageMetadata(
            userId,
            {
              url: image.url,
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const imageMetadata = await storage.platform.ai.updateImageMetadata(
        userId,
        imageId,
        {
          altText,
 ...
 */
router.get(
  "/vision/alt-text/:id/suggestions",
  isAuthenticated,
  async (req: Request, res: Response) => {
    try {
      const userId = getAuthenticatedUserId(req);
      ...
      const imageMetadata = await storage.platform.ai.getImageMetadata(
        userId,
        imageId,
      );
      if (!imageMetadata) {
    ...
    try {
      ...
      try {
        ...
        const savedTranscription =
          await storage.platform.ai.createTranscription(userId, {
            ...
            status: "completed",
            segments:
              transcription.segments?.map((seg: any, idx: number) => ({
                id: `seg_${idx}`,
                start: seg.start || 0,
                end: seg.end || 0,
                text: seg.text || "",
            ...
            metadata: {
              title:
                title ||
                `Transcription from ${new Date().toLocaleDateString()}`,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const transcription = await storage.platform.ai.getTranscription(
        userId,
        req.params.id,
      );
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const original = await storage.platform.ai.getTranscription(
        userId,
        req.params.id,
      );
      if (!original) {
      ...
      const updated = await storage.platform.ai.updateTranscription(
        userId,
        req.params.id,
        {
 ...
 */
router.get(
  "/voice/transcriptions/:id/export",
  isAuthenticated,
  async (req: Request, res: Response) => {
    try {
      const userId = getAuthenticatedUserId(req);
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="transcription-${req.params.id}.${format}"`,
      );
      res.send(formatted);
    } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const commandRecord = await storage.platform.ai.createVoiceCommand({
        ...
        metadata: { originalCommand: command } as any,
      });
            ...
            if (interpretation.parameters?.items) {
              for (const item of interpretation.parameters.items) {
                await storage.user.inventory.createFoodItem(userId, {
                  ...
                  storageLocationId:
                    interpretation.parameters.location || "default",
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const transcriptionsData = await storage.platform.ai.getTranscriptions(
        userId,
        undefined,
        1000,
      );
      const commands = await storage.platform.ai.getVoiceCommands(
        userId,
        undefined,
        1000,
      );
