...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const preferences =
        await storage.user.scheduling.getSchedulingPreferences(userId);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const preferences =
        await storage.user.scheduling.upsertSchedulingPreferences(
          userId,
          validation.data,
        );
      res.json(preferences);
      ...
      res
        .status(500)
        .json({ error: "Failed to update scheduling preferences" });
    }
  },
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      for (const participantId of allParticipants) {
        const prefs =
          await storage.user.scheduling.getSchedulingPreferences(participantId);
      ...
      for (const participantId of allParticipants) {
        const events = await storage.user.scheduling.getMeetingEvents(
          participantId,
          {
            startTime: now,
            endTime: futureDate,
...
Return as JSON with format:
{
  ...
  "confidence": {
    ...
    "conflictAvoidance": 0-1
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an AI scheduling assistant that finds optimal meeting times considering participant preferences, time zones, and existing commitments.",
          },
          {
            role: "user",
            content: prompt,
      ...
      const suggestions =
        await storage.user.scheduling.createMeetingSuggestions({
          meetingId,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const suggestions =
        await storage.user.scheduling.getUserMeetingSuggestions(
          userId,
...
router.put(
  "/schedule/suggestions/:meetingId",
  isAuthenticated,
  async (req: Request, res: Response) => {
    try {
      const userId = req.user?.id;
      ...
      const suggestion =
        await storage.user.scheduling.getMeetingSuggestions(meetingId);
      if (!suggestion) {
        return res.status(404).json({ error: "Meeting suggestion not found" });
      }

      const updated =
        await storage.user.scheduling.updateMeetingSuggestionStatus(
          meetingId,
          status,
          selectedTime,
        );
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a schedule optimization expert that helps improve productivity and well-being.",
          },
          {
            role: "user",
            content: prompt,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const analytics =
        await storage.user.scheduling.analyzeSchedulingPatterns(userId);
      ...
      const avgMeetingDuration =
        totalMeetings > 0 ? totalHours / totalMeetings : 0;
      ...
      const peakHour = Object.entries(hourDistribution).sort(
        (a, b) => b[1] - a[1],
      )[0];
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const event = await storage.user.scheduling.createMeetingEvent(
        validation.data,
      );
      res.json(event);
    } catch (error) {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updated = await storage.user.scheduling.updateMeetingEvent(
        id,
        req.body,
      );
      res.json(updated);
