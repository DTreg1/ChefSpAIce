...
router.get("/test", async (_req: Request, res: Response) => {
  try {
    ...
    const score =
      await intelligentNotificationService.scoreNotificationRelevance(
        "test-user",
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const preferences =
        await storage.user.notifications.getNotificationPreferences(userId);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const updated =
        await storage.user.notifications.upsertNotificationPreferences(
          prefsToUpdate,
        );
      res.json(updated);
    } catch (error) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const scoreData =
        await intelligentNotificationService.processNotification(
          userId,
      ...
      const score =
        await storage.user.notifications.createNotificationScore(scoreData);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const feedback =
        await storage.user.notifications.createNotificationFeedback({
          userId,
      ...
      setImmediate(async () => {
        try {
          const recentFeedback =
            await storage.user.notifications.getNotificationFeedback(userId);
          if (recentFeedback.length >= 20) {
            await intelligentNotificationService.updateModelWithFeedback(
              recentFeedback,
            );
          }
        } catch (error) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const engagement =
        await storage.user.notifications.getRecentUserEngagement(userId, days);

      // Get notification scores for additional insights
      const scores =
        await storage.user.notifications.getNotificationScores(userId);

      // Calculate average relevance scores (use score or relevanceScore)
      const avgRelevanceScore =
        scores.length > 0
          ? scores.reduce(
              (sum, s) => sum + (s.relevanceScore ?? s.score ?? 0),
              0,
            ) / scores.length
          : 0;
      ...
      const timingAccuracy =
        sentScores.length > 0
          ? sentScores.filter((s) => {
              ...
              const optimalTime = s.holdUntil
                ? new Date(s.holdUntil).getTime()
                : actualTime;
      ...
      const clickRate =
        totalSent > 0 ? (engagement.clicked || 0) / totalSent : 0;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const scores =
        await storage.user.notifications.getNotificationScores(userId);
      const engagement =
        await storage.user.notifications.getRecentUserEngagement(userId, 30);
      ...
      scores.forEach((score) => {
        ...
        const wasClicked = feedbackList.some(
          (f) => f.notificationId === score.id && f.feedbackType === "useful",
        );
        if (wasClicked) {
          perf.clicked++;
        }
      ...
      feedbackList
        .filter((f) => f.feedbackType === "useful")
        .forEach((f) => {
      ...
      scores
        .filter((s) => s.actualSentAt)
        .forEach((s) => {
          const hour = new Date(s.actualSentAt!).getHours();
          sentByHour[hour]++;
        });

      const hourCTR = clicksByHour
        .map((clicks, hour) => ({
          hour,
      ...
      const clickRateInsights =
        totalSentForInsights > 0
          ? (engagement.clicked || 0) / totalSentForInsights
          : 0;
      ...
      if (clickRateInsights > 0) {
        ...
        const percentToTarget =
          (currentImprovement / (improvementTarget - 0.2)) * 100;
