          ...
          async () => {
            try {
              if (!streamingEnabled) {
                ...
                const content =
                  await chatService.createChatCompletion(messages);
              ...
              if (accumulatedContent) {
                ...
                const detectedTerms =
                  await chatService.detectCookingTerms(accumulatedContent);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const message = await storage.user.chat.createChatMessage(
        userId,
        validation.data,
      );
      res.json(message);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        const aiResponse = await chatService.createChatCompletion(
          context.messages,
        );

        // Save AI response with conversationId in metadata
        const assistantMessage = await storage.user.chat.createChatMessage(
          userId,
          {
            role: "assistant",
            content: aiResponse,
