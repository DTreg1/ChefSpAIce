...
const getShoppingListHandler = async (req: Request, res: Response) => {
  try {
    ...
    const shoppingData =
      await storage.user.inventory.getGroupedShoppingItems(userId);
    const totalItems = shoppingData.items?.length || 0;
    const checkedItems = (
      shoppingData.items?.filter((i: any) => i.isPurchased) ?? []
    ).length;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const mealPlans = await storage.user.recipes.getMealPlans(
        userId,
        startDate,
        endDate,
      );
      ...
      if (mealPlans.length === 0) {
        sendProgress(100, "No meal plans found for the selected date range", {
          itemsAdded: 0,
        });
        res.end();
        return;
      ...
      const existingShoppingItems =
        await storage.user.inventory.getShoppingItems(userId);
      const existingItemNames = new Set(
        existingShoppingItems.map((item: any) =>
          (item.name || item.ingredient || "").toLowerCase(),
        ),
      );
      ...
      ingredientsByRecipe.forEach((ingredients, recipeId) => {
        ingredients.forEach((ingredient) => {
          const cleanIngredient = ingredient
            .replace(/^\d+[\s\d]*/, "")
            .replace(
              /^(cup|cups|tbsp|tablespoon|tsp|teaspoon|oz|ounce|lb|pound|g|gram|kg|ml|l|liter)s?\s+/i,
              "",
            )
            .trim();
          ...
          if (
            !inventoryNames.has(ingredientLower) &&
      ...
      if (missingItems.length === 0) {
        sendProgress(
          100,
          "All ingredients are already in inventory or shopping list!",
          { itemsAdded: 0 },
        );
        res.end();
        return;
      ...
      for (let i = 0; i < missingItems.length; i += batchSize) {
        ...
        const progressPercent =
          80 + (20 * (i + batch.length)) / missingItems.length;
        sendProgress(
