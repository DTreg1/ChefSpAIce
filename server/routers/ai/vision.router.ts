...
/**
 * GET /api/ai/vision/alt-text
 * Get all alt texts for the user
 */
router.get("/alt-text", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    
    const { limit = 50, offset = 0, quality = "all" } = req.query;
    
    const altTexts = await storage.platform.ai.getAllAltTexts(userId);
    
    // Filter by quality if specified
    let filtered = altTexts;
    if (quality !== "all") {
      filtered = await Promise.all(
        altTexts.map(async (item) => {
          const qualityScore = await analyzeAltTextQuality(item.text);
          return { ...item, qualityScore };
        })
      ).then(items => {
        if (quality === "excellent") return items.filter(i => i.qualityScore.score >= 90);
        if (quality === "good") return items.filter(i => i.qualityScore.score >= 70 && i.qualityScore.score < 90);
        if (quality === "fair") return items.filter(i => i.qualityScore.score >= 50 && i.qualityScore.score < 70);
        if (quality === "poor") return items.filter(i => i.qualityScore.score < 50);
        return items;
      });
    }
    ...
    res.status(500).json({ error: "Failed to fetch alt texts" });
  }
});

/**
 * DELETE /api/ai/vision/alt-text/:id
 * Delete an alt text entry
 */
router.delete("/alt-text/:id", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    
    await storage.platform.ai.deleteAltText(userId, req.params.id);
    res.status(204).send();
  } catch (error) {
    console.error("Error deleting alt text:", error);
    res.status(500).json({ error: "Failed to delete alt text" });
  }
});

/**
 * GET /api/ai/vision/alt-text/accessibility-dashboard
 * Get accessibility dashboard with metrics and insights
 */
router.get("/alt-text/accessibility-dashboard", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    ...
    const metrics = {
      ...
      withAltText: altTexts.filter(a => a.text && a.text.length > 0).length,
      withoutAltText: altTexts.filter(a => !a.text || a.text.length === 0).length,
      decorative: altTexts.filter(a => a.isDecorative).length,
      averageLength: Math.round(
        altTexts.reduce((sum, a) => sum + (a.text?.length || 0), 0) / altTexts.length
      ),
    };
    
    // Calculate compliance score
    const complianceScore = Math.round((metrics.withAltText / metrics.totalImages) * 100) || 0;
    
    // Get quality distribution
    const qualityScores = await Promise.all(
      altTexts.map(async (item) => {
        const quality = await analyzeAltTextQuality(item.text);
    ...
    res.json({
      ...
      insights: {
        strengths: [
          complianceScore > 80 ? "High alt text coverage" : null,
          averageQualityScore > 70 ? "Good overall quality" : null,
          metrics.averageLength > 50 && metrics.averageLength < 150 ? "Appropriate text lengths" : null,
        ].filter(Boolean),
        improvements: [
          complianceScore < 80 ? `Add alt text to ${metrics.withoutAltText} images` : null,
          averageQualityScore < 70 ? "Improve alt text quality" : null,
          metrics.averageLength < 50 ? "Add more descriptive details" : null,
          metrics.averageLength > 150 ? "Consider more concise descriptions" : null,
        ].filter(Boolean),
      },
      recommendations: {
        immediate: altTexts
    ...
    console.error("Error generating accessibility dashboard:", error);
    res.status(500).json({ error: "Failed to generate accessibility dashboard" });
  }
});

/**
 * POST /api/ai/vision/alt-text/batch-update
 * Batch update alt texts
 */
router.post("/alt-text/batch-update", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getAuthenticatedUserId(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
    ...
    const results = await Promise.all(
      updates.map(async (update) => {
        try {
          await storage.platform.ai.updateAltText(userId, update.id, update.text);
    ...
    console.error("Error batch updating alt texts:", error);
    res.status(500).json({ error: "Failed to batch update alt texts" });
  }
});
