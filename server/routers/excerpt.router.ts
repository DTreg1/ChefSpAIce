...
export function createExcerptRouter(storage: IStorage): Router {
  ...
  router.post('/generate', async (req, res) => {
    try {
      ...
      for (const generated of generatedExcerpts) {
        const excerpt = await storage.createExcerpt(userId, {
  ...
  router.get('/test', async (req, res) => {
    try {
      ...
      const excerpts = await storage.getExcerptsByContent(userId, contentId);
  ...
  router.get('/performance', async (req, res) => {
    try {
      ...
      const performance = await storage.getExcerptPerformance(excerptId, start, end);
  ...
  router.put('/optimize', async (req, res) => {
    try {
      ...
      // Get current excerpt
      const excerpts = await storage.getExcerptsByContent(userId, validatedData.excerptId);
      ...
      const performance = await storage.getExcerptPerformance(validatedData.excerptId);
      ...
      const optimizedExcerpt = await storage.createExcerpt(userId, {
  ...
  router.put('/:excerptId/activate', async (req, res) => {
    try {
      ...
      await storage.setActiveExcerpt(userId, contentId, excerptId);
  ...
  router.delete('/:excerptId', async (req, res) => {
    try {
      ...
      const { excerptId } = req.params;
      await storage.deleteExcerpt(userId, excerptId);
  ...
  router.post('/:excerptId/track', async (req, res) => {
    try {
      ...
      const performance = await storage.recordExcerptPerformance({
        excerptId,
