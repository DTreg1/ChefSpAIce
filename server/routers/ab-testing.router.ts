...
// Get all tests
router.get("/", asyncHandler(async (req: ExpressRequest<any, any, any, any>, res: ExpressResponse) => {
  try {
    const filters: any = {};
...
router.get("/results/:id", asyncHandler(async (req: ExpressRequest<any, any, any, any>, res: ExpressResponse) => {
  const { id } = req.params;
...
router.post("/results", asyncHandler(async (req: ExpressRequest<any, any, any, any>, res: ExpressResponse) => {
  try {
    ...
    res.status(400).json({ error: error.message || "Failed to update results" });
  }
}));

// Analyze test for statistical significance
router.post("/analyze", asyncHandler(async (req: ExpressRequest<any, any, any, any>, res: ExpressResponse) => {
      ...
      try {
        ...
        const response = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "You are an expert in A/B testing and conversion rate optimization. Provide clear, actionable insights in plain language."
            },
            {
              role: "user",
              content: prompt
        ...
        for (const line of lines) {
          if (line.toLowerCase().includes("explanation") || line.startsWith("1.")) {
            currentSection = "explanation";
          } else if (line.toLowerCase().includes("findings") || line.toLowerCase().includes("insights") || line.startsWith("2.")) {
            currentSection = "findings";
          } else if (line.toLowerCase().includes("next steps") || line.toLowerCase().includes("recommendations") || line.startsWith("3.")) {
            currentSection = "nextsteps";
          } else if (line.toLowerCase().includes("warning") || line.toLowerCase().includes("consideration") || line.startsWith("4.")) {
            currentSection = "warnings";
          } else if (line.toLowerCase().includes("best practice") || line.startsWith("5.")) {
            ...
            switch(currentSection) {
              case "explanation":
                ...
                break;
              case "findings":
                if (!line.match(/^\d+\./)) keyFindings.push(line.replace(/^[-•*]\s*/, ''));
                break;
              case "nextsteps":
                if (!line.match(/^\d+\./)) nextSteps.push(line.replace(/^[-•*]\s*/, ''));
                break;
              case "warnings":
                if (!line.match(/^\d+\./)) warnings.push(line.replace(/^[-•*]\s*/, ''));
                break;
            }
          }
        }
        ...
        aiInsights = await experimentsStorage.upsertAbTestInsight({
          ...
          recommendation: significance.winner !== 'inconclusive' ? 'implement' : 'continue',
          ...
          statisticalAnalysis: {
            ...
            conversionRateA: aggregated.variantA.visitors > 0 ? aggregated.variantA.conversions / aggregated.variantA.visitors : 0,
            conversionRateB: aggregated.variantB.visitors > 0 ? aggregated.variantB.conversions / aggregated.variantB.visitors : 0,
