...
const testPatterns = [
  ...
  },
  {
    id: 'fraud-3',
    type: 'card_testing',
    metadata: { failedTransactions: 20, successfulTransactions: 1, timeWindow: 600 },
    expectedFraud: true,
  ...
  },
  {
    ...
    metadata: { amount: 50000, userAverageSpend: 100, itemCategory: 'electronics' },
    expectedFraud: true,
  ...
  },
  {
    id: 'fraud-8',
    type: 'identity_theft',
    metadata: { nameChange: true, addressChange: true, immediateTransaction: true },
    expectedFraud: true,
  ...
  },
  {
    id: 'fraud-9',
    type: 'bot_behavior',
    metadata: { actionsPerMinute: 100, mouseMovement: 'linear', keyboardTiming: 'uniform' },
    expectedFraud: true,
  ...
  },
  {
    ...
    metadata: { consistentTime: true, consistentAmount: true, consistentVendor: true },
    expectedFraud: false,
...
async function validateAccuracy(): Promise<void> {
  ...
  for (const pattern of testPatterns) {
    ...
    if (isCorrect) {
      correctDetections++;
      console.log(`✅ ${pattern.id}: Correctly identified as ${pattern.expectedFraud ? 'FRAUD' : 'LEGITIMATE'}`);
      console.log(`   Score: ${(result.fraudScore * 100).toFixed(1)}% | ${pattern.description}`);
    } else {
      if (detectedAsFraud && !pattern.expectedFraud) {
        falsePositives++;
        console.log(`❌ ${pattern.id}: FALSE POSITIVE - Legitimate flagged as fraud`);
        console.log(`   Score: ${(result.fraudScore * 100).toFixed(1)}% | ${pattern.description}`);
      } else {
        falseNegatives++;
  ...
  const precision = correctDetections / (correctDetections + falsePositives) * 100;
  const recall = correctDetections / (correctDetections + falseNegatives) * 100;
