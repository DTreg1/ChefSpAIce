...
export async function ensureDefaultStorageLocations(
  ...
  const existingNames = new Set(existingLocations.map((l: UserStorage) => l.name));
  ...
  for (const locConfig of locationsToCreate) {
    try {
      ...
      console.error(`Failed to create default storage location ${locConfig.name} for user ${userId}:`, error);
    }
  }
...
export async function resolveStorageLocationId(
  ...
  if (!DEFAULT_LOCATIONS.find(loc => loc.name === normalizedName)) {
    try {
      ...
      console.error(`Failed to create custom storage location ${normalizedName} for user ${userId}:`, error);
      return null;
    }
  }
  ...
  if (locationMap.size > 0) {
    console.warn(`Could not resolve storage location "${locationName}", using first available location`);
...
export async function getDefaultStorageLocationId(
  ...
  return locationMap.get('Refrigerator') || 
