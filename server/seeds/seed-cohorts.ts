...
export async function seedCohorts(storage: typeof defaultStorage) {
  ...
  try {
    ...
    // Record all metrics
    await (storage as any).recordCohortMetrics([
      ...januaryMetrics,
      ...februaryMetrics,
      ...marchMetrics,
    ]);
    ...
    const januaryInsights: InsertCohortInsight[] = [
      {
        cohortId: januaryCohort.id,
        insight:
          "January signups show exceptional 50% higher retention than February cohort. The organic acquisition channel appears to attract more engaged users who find genuine value in the product. Focus marketing efforts on organic channels like SEO and content marketing to replicate January's success.",
      ...
      },
      {
        cohortId: januaryCohort.id,
        insight:
          "Users from January cohort demonstrate strong product adoption patterns with 7-minute average sessions, suggesting they're finding value in core features. Interview January users to understand what drove their engagement and document their use cases.",
    ...
    const februaryInsights: InsertCohortInsight[] = [
      {
        cohortId: februaryCohort.id,
        insight:
          "February cohort shows concerning 35% 30-day retention, indicating potential mismatch between paid ad messaging and actual product value. Early churn risk is critical. Immediately review and align paid search ad copy with actual product capabilities. Implement better onboarding for paid acquisition users.",
        insightType: "churn",
        confidence: 0.89,
        impact: "critical",
      },
      {
        cohortId: februaryCohort.id,
        insight:
          "Despite higher acquisition volume (200 users vs 150), February's paid search cohort generates lower overall value due to poor retention and engagement. Re-evaluate paid search ROI considering lifetime value, not just acquisition cost. Consider shifting budget to organic channels.",
        insightType: "retention",
        confidence: 0.91,
        impact: "high",
      },
    ...
    const marchInsights: InsertCohortInsight[] = [
      {
        cohortId: marchCohort.id,
        insight:
          "March's social media acquisition shows moderate performance between January's organic and February's paid search, suggesting potential for optimization. Test different social media platforms and content types to identify which drive higher-quality users similar to January's organic cohort.",
        insightType: "growth",
        confidence: 0.85,
        impact: "medium",
      },
    ...
    // Save all insights
    await Promise.all([
      ...januaryInsights.map((insight) =>
        (storage as any).createCohortInsight(insight),
      ),
      ...februaryInsights.map((insight) =>
        (storage as any).createCohortInsight(insight),
      ),
      ...marchInsights.map((insight) =>
        (storage as any).createCohortInsight(insight),
      ),
    ]);
    ...
    console.log(
      "  - Demonstrated 50% higher retention for January vs February",
    );
