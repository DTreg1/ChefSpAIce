...
async function seedAbTests() {
  ...
  const tests: InsertAbTest[] = [
    {
      ...
      configuration: {
        controlGroup: {
          size: 0.5,
          features: { buttonColor: "blue", buttonText: "Sign Up" },
        },
        variants: [
          {
            name: "variant_b",
            size: 0.5,
            features: { buttonColor: "green", buttonText: "Get Started" },
          },
        ],
        targetingCriteria: {
    ...
    },
    {
      ...
      configuration: {
        controlGroup: {
          size: 0.5,
          features: { formType: "single-page", allFieldsVisible: true },
        },
        variants: [
          {
            name: "variant_b",
            size: 0.5,
            features: { formType: "multi-step", progressIndicator: true },
          },
        ],
        targetingCriteria: {
    ...
    },
    {
      ...
      configuration: {
        ...
        variants: [
          { name: "variant_b", size: 0.5, features: { layout: "grid" } },
        ],
        targetingCriteria: {
          featureArea: "recipes",
  ...
  for (let i = 0; i < 50; i++) {
    results.push({
      ...
      metadata: {
        buttonClicks: Math.floor(Math.random() * 5),
  ...
  for (let i = 0; i < 100; i++) {
    ...
    results.push({
      ...
      converted: isControl ? Math.random() > 0.75 : Math.random() > 0.6,
      metadata: {
        formCompletionTime: isControl
  ...
  const variantMetrics: InsertAbTestVariantMetric[] = [
    ...
    },
    {
      ...
      calculatedAt: new Date(),
      recommendation:
        "Implement variant B - Multi-step form shows significant improvement with 60% lift",
    },
  ];
...
async function seedCohorts() {
  ...
  const januaryCohort = await storage.admin.experiments.createCohort({
    ...
    criteria: {
      userAttributes: {
        signupDateRange: { start: "2025-01-01", end: "2025-01-31" },
        source: "organic",
      },
    },
    userCount: 150,
  ...
  const februaryCohort = await storage.admin.experiments.createCohort({
    ...
    criteria: {
      userAttributes: {
        signupDateRange: { start: "2025-02-01", end: "2025-02-28" },
        source: "paid_search",
      },
    },
    userCount: 200,
  ...
  const marchCohort = await storage.admin.experiments.createCohort({
    ...
    criteria: {
      userAttributes: {
        signupDateRange: { start: "2025-03-01", end: "2025-03-31" },
        source: "social_media",
      },
    },
    userCount: 175,
  ...
  const insights = [
    {
      ...
      insight:
        "January cohort shows 58% retention at day 30, outperforming historical average by 12%. Strong Day-30 Retention indicates effective onboarding.",
      confidence: 0.92,
      impact: "high" as const,
      recommendations: [
        "Continue current onboarding flow",
        "A/B test email cadence",
      ],
    },
    {
      ...
      insight:
        "February cohort experiences 13% drop between day 1 and day 7, higher than expected 8%. Week 1 engagement needs improvement.",
      confidence: 0.85,
    ...
    },
    {
      ...
      insight:
        "March cohort from social media shows strong initial engagement at 82% day 1 retention. Social acquisition channel performing well.",
      confidence: 0.78,
      impact: "medium" as const,
      recommendations: [
...
async function runAllSeeds() {
  ...
  try {
    ...
    console.log(
      `   A/B Tests: ${abTestResults.tests} tests, ${abTestResults.results} results, ${abTestResults.metrics} metrics`,
    );
    console.log(
      `   Cohorts: ${cohortResults.cohorts} cohorts, ${cohortResults.insights} insights\n`,
    );
