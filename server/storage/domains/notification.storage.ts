...
import type {
  ...
  NotificationStats,
} from "../interfaces/INotificationStorage";
...
export class NotificationStorage implements INotificationStorage {
  ...
  async getUserNotificationFeedback(
    userId: string,
  ): Promise<NotificationFeedback[]> {
    return await db
      .select()
  ...
  async getNotificationStats(
    ...
    const delivered = allNotifications.filter(
      (n) => n.deliveredAt !== null,
    ).length;
    const opened = allNotifications.filter((n) => n.openedAt !== null).length;
    const dismissed = allNotifications.filter(
      (n) => n.dismissedAt !== null,
    ).length;
    ...
    return {
      ...
      deliveryRate:
        allNotifications.length > 0 ? delivered / allNotifications.length : 0,
