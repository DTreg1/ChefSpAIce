...
export class AnalyticsStorage implements IAnalyticsStorage {
  ...
  async getWebVitalsByMetric(
    ...
    if (userId) {
      return await db
        .select()
        .from(webVitals)
        .where(and(eq(webVitals.metric, metric), eq(webVitals.userId, userId)))
        .orderBy(desc(webVitals.timestamp));
    }
  ...
  async getWebVitalsStats(
    ...
    const vitals = userId
      ...
      : await db
          .select()
          .from(webVitals)
          .where(gte(webVitals.timestamp, startDate));

    // Calculate percentiles for each metric
    const metricGroups = vitals.reduce(
      (acc: Record<string, number[]>, vital) => {
        if (!acc[vital.metric]) {
          acc[vital.metric] = [];
        }
    ...
    switch (type) {
      case "sessions":
        ...
        return {
          ...
          averageDuration:
            sessions.reduce((sum: number, s: UserSession) => {
              if (s.endedAt) {
                return (
                  sum +
          ...
          activeSessions: sessions.filter((s: UserSession) => !s.endedAt)
            .length,
        };
  ...
  async getPredictionAccuracy(
    ...
    return await db
      ...
      .where(
        sql`${predictionAccuracy.predictionId} = ANY(${sql.raw(`ARRAY[${predictionIds.map((id: string) => `'${id}'`).join(",")}]`)})`,
      )
  ...
  async updateTrendAlert(
    alertId: string,
    updates: Partial<InsertTrendAlert>,
  ): Promise<TrendAlert> {
    const [result] = await db
      .update(trendAlerts)
      .set(updates)
