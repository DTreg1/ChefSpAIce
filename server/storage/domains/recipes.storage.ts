...
export class RecipesDomainStorage implements IRecipesStorage {
  ...
  async getRecipes(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting recipes for user ${userId}:`,
        error,
      );
  ...
  async getRecipesPaginated(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting paginated recipes for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async getRecipe(userId: string, id: string): Promise<Recipe | undefined> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting recipe ${id} for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async searchRecipes(userId: string, query: string): Promise<Recipe[]> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error searching recipes for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async searchRecipesByIngredients(
    ...
    try {
      ...
      const rows = await db
        ...
        .from(userRecipes)
        .where(and(eq(userRecipes.userId, userId), or(...ingredientConditions)))
      ...
      console.error(
        `[${DOMAIN}] Error searching recipes by ingredients for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async createRecipe(userId: string, recipe: InsertRecipe): Promise<Recipe> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error creating recipe for user ${userId}:`,
        error,
      );
      const originalError =
  ...
  async updateRecipe(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error updating recipe ${id} for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async deleteRecipe(userId: string, id: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error deleting recipe ${id} for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async toggleRecipeFavorite(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error toggling favorite for recipe ${id}:`,
        error,
      );
    ...
    try {
      ...
      return allRecipes.filter((recipe) => {
        ...
        const overlapPercentage =
          ...
          Math.max(ingredients.length, recipeIngredients.length);

        return overlapPercentage >= 0.7;
      });
    } catch (error) {
      console.error(
        `[${DOMAIN}] Error finding similar recipes for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getMealPlans(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting meal plans for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async getMealPlansByDate(userId: string, date: string): Promise<MealPlan[]> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting meal plans for date ${date}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async markMealPlanCompleted(userId: string, id: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error marking meal plan ${id} as completed:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getMostUsedRecipes(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting most used recipes for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async getRecipeCategories(userId: string): Promise<string[]> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting recipe categories for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getRecipeCuisines(userId: string): Promise<string[]> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting recipe cuisines for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
    ...
    try {
      ...
      const scoredRecipes = allRecipes.map((recipe) => {
        ...
        return {
          recipe,
          score:
            availableIngredients.length / Math.max(recipeIngredients.length, 1),
      ...
      console.error(
        `[${DOMAIN}] Error getting recipe suggestions based on inventory for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getRecipeSuggestionsBasedOnExpiring(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting recipe suggestions based on expiring items for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
