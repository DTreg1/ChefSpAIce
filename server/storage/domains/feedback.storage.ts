...
} from "../interfaces/IFeedbackStorage";

export class FeedbackStorage implements IFeedbackStorage {
  // Feedback Management
  async createFeedback(
    feedbackData: InsertUserFeedback,
  ...
  async getCommunityFeedbackForUser(
    ...
    return await db
      ...
      .from(userFeedback)
      .where(
        and(eq(userFeedback.userId, userId), eq(userFeedback.isPublic, true)),
      )
      .orderBy(desc(userFeedback.createdAt))
      .limit(limit);
  }
  ...
  async addFeedbackResponse(
    responseData: InsertFeedbackResponse,
  ...
  async getFeedbackAnalytics(
    ...
    const respondedFeedback = allFeedback.filter(
      (f: UserFeedback) => f.response && f.respondedAt,
    );
    ...
    for (const f of respondedFeedback) {
      if (f.respondedAt && f.createdAt) {
        totalResponseTime +=
          new Date(f.respondedAt).getTime() - new Date(f.createdAt).getTime();
      }
    }
    ...
    const feedbackWithUpvotes = allFeedback.filter((f: UserFeedback) => {
      const upvotes = f.upvotes as Array<{
        userId: string;
        createdAt: string;
      }> | null;
    ...
    const upvoteRate =
      allFeedback.length > 0 ? feedbackWithUpvotes / allFeedback.length : 0;
  ...
  async upvoteFeedback(userId: string, feedbackId: string): Promise<void> {
    ...
    const currentUpvotes =
      (feedback.upvotes as Array<{
        userId: string;
        createdAt: string;
      }> | null) || [];

    // Check if user already upvoted
    const alreadyUpvoted = currentUpvotes.some(
  ...
  async removeUpvote(userId: string, feedbackId: string): Promise<void> {
    ...
    const newUpvotes = currentUpvotes.filter(
      (upvote: { userId: string }) => upvote.userId !== userId,
    );
