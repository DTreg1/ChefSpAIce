...
export class UserAuthDomainStorage implements IUserStorage {
  ...
  async getUserByPrimaryProviderId(
    provider: string,
    providerId: string,
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting user by provider ${provider}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
    ...
    try {
      const [newUser] = await db
        ...
        .values({
          ...
          storageAreasEnabled: userData.storageAreasEnabled || [
            "Fridge",
            "Pantry",
          ],
      ...
      if (
        originalError.message.includes("unique") ||
        originalError.message.includes("duplicate")
      ) {
  ...
  async updateUserPreferences(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error updating preferences for user ${userId}:`,
        error,
      );
  ...
  async updateUserNotificationPreferences(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error updating notification preferences for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async markOnboardingComplete(userId: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error marking onboarding complete for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async linkOAuthProvider(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error linking OAuth provider for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async unlinkOAuthProvider(userId: string, provider: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error unlinking OAuth provider for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getAuthProviderByProviderAndId(
    provider: string,
    providerId: string,
  ): Promise<AuthProviderInfo | undefined> {
    const context: StorageErrorContext = {
      domain: DOMAIN,
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting auth provider for ${provider}/${providerId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async getAuthProviderByProviderAndUserId(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting auth provider for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async createAuthProvider(
    provider: InsertAuthProviderInfo,
    ...
    try {
      ...
      if (user) {
        ...
        (updates as Record<string, unknown>)[providerField] =
          provider.providerId;
        ...
        const userToCreate: Partial<User> = {
          email:
            email || `${provider.provider}_${provider.providerId}@oauth.local`,
          firstName: provider.displayName || email?.split("@")[0] || "User",
          lastName: "User",
          primaryProvider: provider.provider,
        };
        (userToCreate as Record<string, unknown>)[providerField] =
          provider.providerId;
  ...
  async updateAuthProvider(
    id: string,
    updates: UpdateAuthProviderInfo,
  ): Promise<AuthProviderInfo> {
    const context: StorageErrorContext = {
      domain: DOMAIN,
    ...
    try {
      ...
      if (!user) {
        throw new StorageNotFoundError(`User with ID ${id} not found`, context);
      }

      if (updates.providerId && updates.provider) {
        const providerField = `${updates.provider}Id`;
        const userUpdates: Partial<User> = {};
        (userUpdates as Record<string, unknown>)[providerField] =
          updates.providerId;
      ...
      return {
        ...
        provider:
          updates.provider ||
          (user.primaryProvider as AuthProviderInfo["provider"]) ||
          "email",
      ...
      console.error(
        `[${DOMAIN}] Error updating auth provider for user ${id}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async updateUserAdminStatus(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error updating admin status for user ${userId}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async getUserPreferences(
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting preferences for user ${userId}:`,
        error,
      );
      if (error instanceof StorageError) throw error;
      throw wrapDatabaseError(error, context);
  ...
  async getUsersByProvider(provider: string): Promise<User[]> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error getting users by provider ${provider}:`,
        error,
      );
      throw wrapDatabaseError(error, context);
    }
  ...
  async ensureDefaultDataForUser(userId: string): Promise<void> {
    ...
    try {
      ...
      console.error(
        `[${DOMAIN}] Error ensuring default data for user ${userId}:`,
        error,
      );
    }
  }
