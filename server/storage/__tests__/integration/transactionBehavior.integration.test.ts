...
describe("Transaction Behavior Integration Tests", () => {
  ...
  describe("Atomic Operations", () => {
    ...
    it("should atomically create recipe with all components", async function () {
      ...
      const retrieved = await storage.user.recipes.getRecipe(
        user.id,
        recipe.id,
      );
  ...
  describe("Multi-Step Operations", () => {
    it("should handle user creation and recipe creation in sequence", async function () {
      ...
      const recipe1 = await storage.user.recipes.createRecipe(
        user.id,
        recipe1Data,
      );
      const recipe2 = await storage.user.recipes.createRecipe(
        user.id,
        recipe2Data,
      );
      const recipe3 = await storage.user.recipes.createRecipe(
        user.id,
        recipe3Data,
      );
    ...
    it("should handle recipe creation and meal plan creation in sequence", async function () {
      ...
      const plansForDay = await storage.user.recipes.getMealPlansByDate(
        user.id,
        today,
      );
  ...
  describe("Cascading Behavior", () => {
    it("should maintain referential integrity in meal plans", async function () {
      ...
      const retrievedPlan = await storage.user.recipes.getMealPlan(
        user.id,
        plan.id,
      );
  ...
  describe("Concurrent-Like Operations", () => {
    ...
    it("should handle multiple rapid recipe creations", async function () {
      ...
      const allRecipes = await storage.user.recipes.getRecipes(user.id);
      const rapidRecipes = allRecipes.filter((r) =>
        r.title.startsWith("Rapid Recipe"),
      );
  ...
  describe("Isolation Testing", () => {
    it("should isolate user operations completely", async function () {
      ...
      const recipeData1 = testFactories.recipe(user1.id, {
        title: "User1 Private Recipe",
      });
      const recipeData2 = testFactories.recipe(user2.id, {
        title: "User2 Private Recipe",
      });
    ...
    it("should not leak data between users during concurrent reads", async function () {
      ...
      const user1Titles = retrieved1.filter((r) =>
        r.title.startsWith("User1Recipe"),
      );
      const user2Titles = retrieved2.filter((r) =>
        r.title.startsWith("User2Recipe"),
      );
      const user1HasUser2 = retrieved1.some((r) =>
        r.title.startsWith("User2Recipe"),
      );
      const user2HasUser1 = retrieved2.some((r) =>
        r.title.startsWith("User1Recipe"),
      );
  ...
  describe("Consistent Read After Write", () => {
    it("should read updated data immediately after write", async function () {
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        const retrieved = await storage.getUserById(user.id);
        assert.strictEqual(
          retrieved?.householdSize,
          i,
          `Iteration ${i} should show householdSize=${i}`,
        );
      }
    });
    ...
    it("should read created recipe immediately after creation", async function () {
      ...
      for (let i = 1; i <= 3; i++) {
        const recipeData = testFactories.recipe(user.id, {
          title: `Sequential Recipe ${i}`,
        });
        const created = await storage.user.recipes.createRecipe(
          user.id,
          recipeData,
        );
  ...
  describe("Deletion Consistency", () => {
    it("should not find deleted recipes", async function () {
      ...
      const beforeDelete = await storage.user.recipes.getRecipe(
        user.id,
        recipe.id,
      );
      ...
      const afterDelete = await storage.user.recipes.getRecipe(
        user.id,
        recipe.id,
      );
      assert.strictEqual(
