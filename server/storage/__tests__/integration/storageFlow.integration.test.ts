...
describe("Storage Layer Integration Tests", () => {
  ...
  describe("User Preferences Flow Tests", () => {
    it("should update and retrieve user preferences", async function () {
      ...
      assert.deepStrictEqual(
        retrievedPrefs.dietaryRestrictions,
  ...
  describe("Recipe Flow Tests", () => {
    ...
    it("should update recipe", async function () {
      ...
      const updatedRecipe = await storage.user.recipes.updateRecipe(
        user.id,
        createdRecipe.id,
        {
    ...
    it("should toggle recipe favorite", async function () {
      ...
      const toggledRecipe = await storage.user.recipes.toggleRecipeFavorite(
        user.id,
        createdRecipe.id,
      );
      assert.strictEqual(toggledRecipe?.isFavorite, true);

      const toggledAgain = await storage.user.recipes.toggleRecipeFavorite(
        user.id,
        createdRecipe.id,
      );
    ...
    it("should search recipes by title", async function () {
      ...
      const searchResults = await storage.user.recipes.searchRecipes(
        user.id,
        "Unique Integration",
      );
  ...
  describe("Meal Plan Flow Tests", () => {
    it("should create and retrieve meal plan", async function () {
      ...
      const createdPlan =
        await storage.user.recipes.createMealPlan(mealPlanData);
    ...
    it("should get meal plans by date", async function () {
      ...
      const plansForDate = await storage.user.recipes.getMealPlansByDate(
        user.id,
        today,
      );
    ...
    it("should mark meal plan as completed", async function () {
      ...
      const updatedPlan = await storage.user.recipes.getMealPlan(
        user.id,
        createdPlan.id,
      );
