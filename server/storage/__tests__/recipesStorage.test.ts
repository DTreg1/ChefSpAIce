    ...
    describe("getRecipesPaginated", () => {
      ...
      it("should respect limit and offset", async () => {
        ...
        const result = {
          recipes: Array(limit)
            .fill(null)
            .map((_, i) => createMockRecipe({ id: `recipe-${offset + i}` })),
          total: 25,
        };
      ...
      it("should apply filters with pagination", async () => {
        const result = {
          recipes: [createMockRecipe({ isFavorite: true })],
          total: 5,
        };
  ...
  describe("Recipe Search", () => {
    describe("searchRecipes", () => {
      it("should search by title", async () => {
        const recipes = [createMockRecipe({ title: "Pasta Carbonara" })];
      ...
      it("should search by cuisine", async () => {
        const recipes = [createMockRecipe({ cuisine: "italian" })];

        assert.ok(recipes[0].cuisine?.toLowerCase().includes("italian"));
      });

      it("should search by category", async () => {
        const recipes = [createMockRecipe({ category: "main" })];
  ...
  describe("Recipe Analytics", () => {
    describe("getMostUsedRecipes", () => {
      ...
      it("should respect limit parameter", async () => {
        const limit = 5;
        const recipes = Array(limit)
          .fill(null)
          .map((_, i) => createMockRecipe({ title: `Recipe ${i}` }));
  ...
  describe("Recipe Suggestions", () => {
    describe("getRecipeSuggestionsBasedOnInventory", () => {
      ...
      it("should return limited number of suggestions", async () => {
        const limit = 5;
        const suggestions = Array(limit)
          .fill(null)
          .map(() => createMockRecipe());
    ...
    describe("getRecipeSuggestionsBasedOnExpiring", () => {
      it("should prioritize recipes using expiring ingredients", async () => {
        ...
        const matchingExpiringIngredients = recipe.ingredients.filter(
          (ing: string) =>
