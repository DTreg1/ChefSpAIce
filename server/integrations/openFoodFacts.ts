...
export async function searchOFFByBarcode(barcode: string): Promise<{
  ...
  try {
    ...
    const response = await fetch(
      `${OFF_API_BASE}/api/v2/product/${cleanBarcode}.json`,
      {
        headers: {
...
> {
  try {
    const params = new URLSearchParams({
      ...
      page: "1",
      fields:
        "code,product_name,generic_name,brands,categories,nutriments,nutriments_per_serving,serving_size,serving_quantity,ingredients_text,image_front_url,image_url",
    });
...
export async function getCombinedNutrition(
  ...
  if (
    usdaNutrition &&
    usdaNutrition.calories !== undefined &&
    usdaNutrition.calories > 0 &&
    ((usdaNutrition.protein !== undefined && usdaNutrition.protein > 0) ||
      (usdaNutrition.carbohydrates !== undefined &&
        usdaNutrition.carbohydrates > 0) ||
