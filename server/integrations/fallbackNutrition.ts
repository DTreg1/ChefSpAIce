 ...
 */
function averageNutrition(profiles: Partial<NutritionInfo>[]): Partial<NutritionInfo> {
 ...
 */
export function fallbackSearch(query: string, pageSize: number = 20): USDASearchResponse {
  ...
  const foods = variations.map(name => createFallbackFoodItem(name, nutrition));
