...
function getSessionSecret(): string {
  ...
  console.warn(
    "⚠️  Using development session secret. Set SESSION_SECRET for production.",
  );
  return devSecret;
}
...
function formatApplePrivateKey(key: string | undefined): string {
  ...
  console.log(
    `[Apple OAuth] Key formatted: lines=${lineCount}, contentLength=${keyContent.length}`,
  );

  // Valid ES256 P8 key content is typically 118-122 characters (base64)
  if (keyContent.length < 100 || keyContent.length > 200) {
    console.error(
      `[Apple OAuth] WARNING: Key content length unusual (${keyContent.length} chars). Expected ~118-122 chars for ES256 P8 key.`,
    );
  }
...
console.log(
  `[Apple OAuth] Config check: clientID=${appleClientID ? `set (${appleClientID.length} chars)` : "MISSING"}, teamID=${appleTeamID ? `set (${appleTeamID.length} chars)` : "MISSING"}, keyID=${appleKeyID ? `set (${appleKeyID.length} chars)` : "MISSING"}`,
);
...
export function isOAuthConfigured(provider: OAuthProvider | string): boolean {
  switch (provider) {
    ...
    case "replit":
      ...
      return (
        (!!oauthConfig.replit.clientID && !!oauthConfig.replit.clientSecret) ||
