...
const FOOD_CONVERSIONS: {
  [foodName: string]: { [fromUnit: string]: { [toUnit: string]: number } };
} = {
  bacon: {
    ...
    if (match) {
      ...
      if (pattern.source.includes("Mixed number")) {
        ...
        if (
          unit &&
          /^[A-Z]/.test(unit.trim()) &&
          !isValidUnit(unit.trim().toLowerCase())
        ) {
      ...
      if (
        SIZE_DESCRIPTORS.includes(trimmedUnit) &&
        nameWords.length > 0 &&
        isValidUnit(nameWords[0])
      ) {
      ...
      if (nameWords.length > 1) {
        ...
        if (
          SIZE_DESCRIPTORS.includes(potentialDescriptor) &&
          nameWords.length > 1 &&
          isValidUnit(nameWords[1])
        ) {
...
export function ingredientNamesMatch(name1: string, name2: string): boolean {
  const normalize = (str: string) => {
    ...
    if (words.length > 1) {
      ...
      const normalizedDescriptors = SIZE_DESCRIPTORS.map((d) =>
...
export function matchIngredientWithInventory(
  ...
  if (neededUnitType === "weight" && allMatches.length > 0) {
    ...
    matchingItem = allMatches.find(
      (item) => item.weightInGrams && item.weightInGrams > 0,
    );
    if (matchingItem) {
  ...
  const hasValidWeight =
    matchingItem.weightInGrams && matchingItem.weightInGrams > 0;
  ...
  if (neededUnitType === "weight" && hasValidWeight && inventoryQuantity > 0) {
    ...
    if (convertedQuantity !== null) {
      ...
      const percentageAvailable = Math.min(
        100,
        (convertedQuantity / parsed.quantity) * 100,
      );
  ...
  if (foodConvertedQuantity !== null) {
    ...
    const percentageAvailable = Math.min(
      100,
      (foodConvertedQuantity / parsed.quantity) * 100,
    );
  ...
  const sameUnit =
    matchingItem.unit.toLowerCase() === parsed.unit.toLowerCase();
  if (sameUnit) {
    const hasEnough = inventoryQuantity >= parsed.quantity;
    const percentageAvailable = Math.min(
      100,
      (inventoryQuantity / parsed.quantity) * 100,
    );
  ...
  if (matchingItem.unit.toLowerCase() === "piece") {
    const isPieceOrCountable =
      parsed.unit.toLowerCase() === "piece" ||
      parsed.unit === "" ||
