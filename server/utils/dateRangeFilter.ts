...
export function parseDateRange(
  ...
  if (req.query.startDate) {
    ...
    if (!startDate) {
      throw new Error('Invalid startDate format. Use ISO 8601 format (YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss)');
    }
  ...
  if (req.query.endDate) {
    ...
    if (!endDate) {
      throw new Error('Invalid endDate format. Use ISO 8601 format (YYYY-MM-DD or YYYY-MM-DDTHH:mm:ss)');
    }
  ...
  if (!filter.startDate && !filter.endDate && requireDates) {
    throw new Error('At least one date parameter (startDate or endDate) is required');
  }
  
  // If only endDate provided, default startDate to defaultDays before endDate
  if (!filter.startDate && filter.endDate) {
    filter.startDate = new Date(filter.endDate.getTime() - defaultDays * 24 * 60 * 60 * 1000);
  }
...
export function formatDateRange(filter: DateRangeFilter): string {
  ...
  const start = filter.startDate ? filter.startDate.toISOString().split('T')[0] : 'Beginning';
  const end = filter.endDate ? filter.endDate.toISOString().split('T')[0] : 'Now';
