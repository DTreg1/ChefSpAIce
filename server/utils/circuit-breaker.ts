...
export class CircuitBreaker {
  ...
  async execute<T>(fn: () => Promise<T>): Promise<T> {
    ...
    if (currentState === 'half-open' && this.halfOpenAttempts >= this.config.halfOpenMaxAttempts) {
      throw new AIError(
  ...
  private onSuccess(): void {
    ...
    if (this.state === 'half-open') {
      ...
      if (this.successes >= this.config.successThreshold) {
        this.transitionTo('closed', `${this.config.successThreshold} successful requests`);
        this.failures = 0;
        this.successes = 0;
        this.failureTimestamps = [];
      }
  ...
  private onFailure(): void {
    ...
    if (this.state === 'closed') {
      ...
      if (this.failureTimestamps.length >= this.config.failureThreshold) {
        this.transitionTo('open', `${this.config.failureThreshold} failures in monitoring window`);
      }
  ...
  private getTimeUntilReset(): number {
    ...
    const timeSinceLastFailure = Date.now() - this.lastFailureTime;
    const timeUntilReset = Math.max(0, this.config.recoveryTimeout - timeSinceLastFailure);
