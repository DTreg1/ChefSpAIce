...
export class IntelligentNotificationService {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a notification relevance scoring system. Always respond with valid JSON.",
          },
          {
            role: "user",
            content: prompt,
  ...
  async predictOptimalDeliveryTime(
    ...
    try {
      ...
      if (prefs?.quietHoursStart && prefs.quietHoursEnd) {
        const inQuietHours = this.isInQuietHoursSimple(
          now,
          prefs.quietHoursStart,
          prefs.quietHoursEnd,
        );
        if (inQuietHours) {
          const endOfQuietHours = this.getEndOfQuietHoursSimple(
            now,
            prefs.quietHoursEnd,
          );
          return {
      ...
      const userActiveHours = feedbackHistory
        .filter(
          (f: any) =>
            f.feedbackType === "opened" || f.feedbackType === "clicked",
        )
      ...
      for (let hoursAhead = 1; hoursAhead <= 24; hoursAhead++) {
        const futureTime = new Date(
          now2.getTime() + hoursAhead * 60 * 60 * 1000,
        );
      ...
      const defaultDelay =
        notification.urgency === "low" ? 2 * 60 * 60 * 1000 : 0;
      return {
  ...
  private async predictWithModel(
    ...
    return this.heuristicPrediction(
      notification,
      relevanceScore,
      feedbackHistory,
      prefs,
  ...
  private heuristicPrediction(
    ...
    const sortedHours = Array.from(clicksByHour.entries()).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    if (sortedHours.length > 0) {
      ...
      if (relevanceScore > 0.8 && notification.urgency !== "low") {
        const hoursUntilTarget =
          (targetTime.getTime() - now.getTime()) / (60 * 60 * 1000);
  ...
  async processNotification(
    ...
    const { relevanceScore, reasoning, suggestedPersonalization } =
      await this.scoreNotificationRelevance(userId, notification, userContext);

    // Predict optimal delivery time using TensorFlow.js
    const { optimalTime, confidence, reason } =
      await this.predictOptimalDeliveryTime(
        userId,
        notification,
        relevanceScore,
      );
    ...
    try {
      ...
      for (const notification of pendingNotifications as any[]) {
        try {
          ...
          const frequencyLimit = prefs?.metadata?.frequencyLimit as
            | number
            | undefined;
          if (frequencyLimit) {
            ...
            const recentNotifs = await storage.getNotificationScores(
              notification.userId,
            );
            const todayCount = recentNotifs.filter(
              (n: any) =>
            ...
            if (todayCount >= frequencyLimit) {
              console.log(
                `User ${notification.userId} has reached daily notification limit`,
              );
              continue;
            }
          }
          ...
          console.log(
            `Sending notification to user ${notification.userId}: ${notificationInfo}`,
          );
