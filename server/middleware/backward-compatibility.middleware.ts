...
export function backwardCompatibilityMiddleware(
  ...
  if (!isLegacyPath) {
    ...
    // AI conversation messages
      ...
      originalPath.match(/^\/api\/ai\/conversations\/[\w-]+\/messages$/)
    ) {
      isLegacyPath = true;
      newPath = originalPath.replace(
  ...
  if (isLegacyPath) {
    ...
    if (process.env.NODE_ENV === "development") {
      console.warn(
        `[DEPRECATION] ${method} ${originalPath} is deprecated. Use ${method} ${newPath} instead.`,
      );
    }
 ...
 */
export function requestTransformMiddleware(
  req: Request,
  res: Response,
  next: NextFunction,
) {
  ...
  if (
    req.query.query &&
    (req.path.includes("/food-data") || req.path.includes("/fdc"))
  ) {
  ...
  if (req.method === "DELETE" && req.path.includes("/shopping-lists")) {
    if (
      req.path.includes("/clear-checked") ||
      req.query.clearChecked === "true"
    ) {
  ...
  if (
    req.path.match(/\/admin\/users\/[\w-]+\/admin$/) &&
    req.method === "PATCH"
  ) {
    // Transform admin status update to standard user update
    if (!req.body) req.body = {};
    req.body.isAdmin =
      req.body.admin !== undefined ? req.body.admin : req.body.isAdmin;
