...
import { getCircuitBreaker, CircuitBreakerConfig } from "../utils/circuit-breaker";
...
export const circuitBreakers = {
  ...
  openaiGeneration: getCircuitBreaker('openai-generation', openaiConfigs.generation),
  openaiVision: getCircuitBreaker('openai-vision', openaiConfigs.vision),
  openaiRecipe: getCircuitBreaker('openai-recipe-generation', openaiConfigs.standard),
    ...
    try {
      ...
      if (stats.state === 'open') {
        return res.status(503).json({
          ...
          retryAfter: Math.ceil((stats.lastFailureTime! + (config?.recoveryTimeout || 60000) - Date.now()) / 1000)
        });
      }
      ...
      console.error(`Circuit breaker middleware error for ${breakerName}:`, error);
    ...
    try {
      ...
      for (const name of breakerNames) {
        ...
        if (stats.state === 'open') {
          ...
          const retryAfter = Math.ceil((stats.lastFailureTime! + 60000 - Date.now()) / 1000);
...
export function withFallback(
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error(`Circuit breaker fallback error for ${breakerName}:`, error);
      next();
    }
  };
}
...
export async function executeWithBreaker<T>(
  ...
  const circuitBreaker = typeof breaker === 'string' 
...
export function addCircuitBreakerStats(breakerName: string) {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.error(`Error adding circuit breaker stats for ${breakerName}:`, error);
      next();
    }
  };
}