...
console.log(`${colors.bold}${colors.cyan}Push Notification Credentials Verification${colors.reset}\n`);
...
if (fcmServiceAccount) {
  try {
    ...
    if (serviceAccount.project_id && serviceAccount.private_key && serviceAccount.client_email) {
      ...
      if (serviceAccount.project_id === 'dummy-project') {
        console.log(`${warning} Using dummy credentials - push notifications will NOT work`);
      } else {
        fcmConfigured = true;
      }
    } else {
      console.log(`${cross} FCM_SERVICE_ACCOUNT is invalid - missing required fields`);
    }
  } catch (error) {
  ...
  if (fs.existsSync(resolvedPath)) {
    try {
      ...
      console.log(`${cross} File at FCM_SERVICE_ACCOUNT_PATH is not valid JSON`);
    }
  } else {
    console.log(`${cross} FCM_SERVICE_ACCOUNT_PATH file does not exist: ${fcmServiceAccountPath}`);
    console.log(`  Resolved path: ${resolvedPath}`);
  }
} else if (fcmServerKey) {
  console.log(`${warning} FCM_SERVER_KEY is deprecated - use FCM_SERVICE_ACCOUNT instead`);
  console.log(`  See the setup guide for instructions on obtaining service account credentials`);
} else {
  console.log(`${cross} No Firebase credentials configured`);
  console.log(`  Set FCM_SERVICE_ACCOUNT or FCM_SERVICE_ACCOUNT_PATH environment variable`);
}
...
if (!fcmConfigured) {
  ...
  console.log(`  See docs/PUSH_NOTIFICATIONS_SETUP.md for detailed instructions`);
}
...
if (apnsKeyId && apnsTeamId) {
    ...
    if (apnsKeyContent) {
      try {
        ...
        if (decoded.includes('BEGIN PRIVATE KEY')) {
          console.log(`${checkmark} APNS_KEY_CONTENT contains valid P8 key (base64 encoded)`);
          apnsConfigured = true;
        } else if (apnsKeyContent.includes('BEGIN PRIVATE KEY')) {
          console.log(`${checkmark} APNS_KEY_CONTENT contains valid P8 key (plain text)`);
          apnsConfigured = true;
        } else {
          console.log(`${cross} APNS_KEY_CONTENT does not appear to be a valid P8 key`);
        }
      } catch (error) {
      ...
      if (fs.existsSync(resolvedKeyPath)) {
        ...
        if (content.includes('BEGIN PRIVATE KEY')) {
          console.log(`${checkmark} APNS_KEY_FILE points to valid P8 key: ${apnsKeyFile}`);
          apnsConfigured = true;
        } else {
  ...
  if (apnsProduction === 'true') {
    ...
    console.log(`${warning} APNS_PRODUCTION not set (defaulting to Development mode)`);
  }
} else {
  if (!apnsKeyId) {
...
if (fcmConfigured && apnsConfigured) {
  console.log(`${checkmark} ${colors.green}Both Android and iOS push notifications are configured!${colors.reset}`);
  console.log(`  Your app is ready to send push notifications to both platforms.`);
  ...
  console.log(`${cross} ${colors.red}Neither Android nor iOS push notifications are configured${colors.reset}`);
  console.log(`  Follow the setup guide at docs/PUSH_NOTIFICATIONS_SETUP.md to get started.`);
}
