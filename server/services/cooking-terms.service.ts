...
export class CookingTermsService {
    ...
    for (const term of terms) {
      if (term.startIndex >= lastEnd) {
        ...
        if (
          ...
          lastTerm.endIndex - lastTerm.startIndex
        ) {
  ...
  static async getTerm(term: string): Promise<CookingTerm | null> {
    try {
      ...
      return (
        allTerms.find((t) => t.term.toLowerCase() === term.toLowerCase()) ||
        null
      );
    } catch (error) {
  ...
  static async getRelatedTerms(term: string): Promise<CookingTerm[]> {
    try {
      ...
      if (
        !mainTerm ||
        !mainTerm.relatedTerms ||
        mainTerm.relatedTerms.length === 0
      ) {
        return [];
      }
   ...
   */
  static async formatInstructionsWithTerms(
    instructions: string,
  ): Promise<string> {
