...
export class BarcodeLookupService {
  ...
  private extractCapabilities(
    product: BarcodeLookupResponse["products"][0],
  ): string[] {
  ...
  private extractCapacity(product: BarcodeLookupResponse["products"][0]): {
    ...
    return {
      ...
      servingSize: servingMatch
        ? `up to ${servingMatch[1]} servings`
        : undefined,
    };
  }
  ...
  async lookupByBarcode(barcode: string): Promise<BarcodeProduct | null> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Barcode API error: ${response.status} ${response.statusText}`,
        );
      }
  ...
  async lookupBatch(
    barcodes: string[],
  ): Promise<Map<string, BarcodeProduct | null>> {
  ...
  async searchProducts(query: string): Promise<BarcodeProduct[]> {
    try {
      ...
      for (const product of data.products) {
        ...
        const productCacheKey = ApiCacheService.generateKey(
          "barcode",
