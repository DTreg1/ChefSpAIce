  ...
  start(): void {
    ...
    this.modelTrainingTask = cron.schedule("0 */6 * * *", async () => {
      try {
        ...
        const recentFeedback = await db
          ...
          .where(gte(notificationFeedback.createdAt, sevenDaysAgo))
          .limit(100);
        ...
        for (const { userId } of recentFeedback) {
          ...
          if (feedback.length >= 20) {
            ...
            console.log(
              `Updated model for user ${userId} with ${feedback.length} feedback items`,
            );
          }
        }
    ...
    this.cleanupTask = cron.schedule("0 3 * * *", async () => {
      try {
        ...
        await db.delete(notificationScores).where(
          and(
        ...
        await db
          ...
          .where(lte(notificationFeedback.createdAt, ninetyDaysAgo));
