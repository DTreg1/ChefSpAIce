...
import type {
  InsertSentimentResults,
  SentimentResults,
  InsertSentimentTrends,
} from "@shared/schema";
...
class SentimentService {
  ...
  private async initializeTFModel(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "TensorFlow.js sentiment model initialized with semantic weights",
      );
    } catch (error) {
  ...
  private async initializeSentimentWeights(): Promise<void> {
    ...
    try {
      ...
      if (weights.length > 0 && this.sentimentBoundaries) {
        ...
        for (
          let i = 0;
          i < Math.min(this.sentimentBoundaries.positiveEnd, shape[0]);
          i++
        ) {
          if (values[i]) {
        ...
        for (
          let i = this.sentimentBoundaries.positiveEnd;
          i < Math.min(this.sentimentBoundaries.negativeEnd, shape[0]);
          i++
        ) {
          if (values[i]) {
        ...
        const newWeights = tf.tensor2d(values, [shape[0], dim1] as [
          number,
          number,
        ]);
   ...
   */
  private async runTensorFlowInference(
    text: string,
  ...
  async analyzeSentiment(
  ...
  ): Promise<Omit<InsertSentimentAnalysis, "userId">> {
    ...
    const aspectSentiments = await this.getAspectBasedSentiment(
      content,
      contentType,
    );
    ...
    const confidence = this.calculateConfidenceWithTF(
      combinedScores,
      emotionAnalysis,
    );

    // 8. Determine overall sentiment classification with TF integration
    const sentiment = this.classifySentimentWithTF(
      combinedScores,
      emotionAnalysis,
      aspectSentiments,
    );
  ...
  private combineScores(
    ...
    let combined = {
      positive:
        tfScores.positive * tfWeight + basicSentiment.positive * basicWeight,
      negative:
        tfScores.negative * tfWeight + basicSentiment.negative * basicWeight,
      neutral:
        tfScores.neutral * tfWeight + basicSentiment.neutral * basicWeight,
    };

    // Add OpenAI scores if available
    if (emotionAnalysis?.sentimentScores) {
      combined.positive +=
        emotionAnalysis.sentimentScores.positive * openaiWeight;
      combined.negative +=
        emotionAnalysis.sentimentScores.negative * openaiWeight;
      combined.neutral +=
        (1 -
          emotionAnalysis.sentimentScores.positive -
          emotionAnalysis.sentimentScores.negative) *
        openaiWeight;
    } else {
      ...
      combined.positive =
        combined.positive +
        basicSentiment.positive * redistributeWeight +
        tfScores.positive * redistributeWeight;
      combined.negative =
        combined.negative +
        basicSentiment.negative * redistributeWeight +
        tfScores.negative * redistributeWeight;
      combined.neutral =
        combined.neutral +
        basicSentiment.neutral * redistributeWeight +
        tfScores.neutral * redistributeWeight;
    }
...
Respond in JSON format:
{
   ...
   */
  private async getAspectBasedSentiment(
    content: string,
    contentType?: string,
    ...
    if (
      !contentType ||
      !["review", "feedback", "comment"].includes(contentType)
    ) {
      return {};
    }
...
Respond in JSON format: {"aspect_name": "sentiment"}`,
  ...
  private calculateConfidenceWithTF(
    ...
    const scores = [
      combinedScores.positive,
      combinedScores.negative,
      combinedScores.neutral,
    ];
  ...
  private classifySentiment(
    ...
    const positiveEmotions =
      (emotions.happy || 0) +
      (emotions.excited || 0) +
      (emotions.satisfied || 0);
    const negativeEmotions =
      (emotions.sad || 0) +
      (emotions.angry || 0) +
      (emotions.frustrated || 0) +
      (emotions.disappointed || 0);
  ...
  private classifySentimentWithTF(
    ...
    if (positive === maxScore && positive - negative > dominanceThreshold) {
      ...
      negative === maxScore &&
      negative - positive > dominanceThreshold
    ) {
      return "negative";
  ...
  private async updateTrendForPeriod(
    ...
    console.log(
      `Sentiment trend tracking not yet implemented for ${periodType} ${timePeriod}`,
    );
  }
