...
export class ModerationService {
   ...
   */
  private calculateSeverity(scores: {
    [key: string]: number;
  ...
  private determineAction(
    ...
    const hasHighRisk = highRiskCategories.some(
      (cat) => (scores[cat] || 0) > 0.5,
    );
    ...
    const hasViolations = Object.values(scores).some(
      (score) => score > TOXICITY_THRESHOLDS.low,
    );
  ...
  private getViolatedCategories(scores: { [key: string]: number }): string[] {
    ...
    Object.entries(scores).forEach(([category, score]) => {
      const threshold =
        OPENAI_THRESHOLDS[category as keyof typeof OPENAI_THRESHOLDS] ||
        TOXICITY_THRESHOLDS.medium;
      if (score > threshold) {
  ...
  private generateMessage(
    ...
    const violations = categories
      .map(
        (cat) =>
          categoryMessages[cat] || cat.replace(/([A-Z])/g, " $1").toLowerCase(),
      )
      .filter(Boolean);
  ...
  private generateSuggestions(
    ...
    if (categories.includes("hate")) {
      ...
      suggestions.push(
        "Avoid generalizations about people based on their identity",
      );
    }
    ...
    if (suggestions.length === 0 && categories.length > 0) {
      suggestions.push(
        "Review your content for potentially inappropriate material",
      );
      suggestions.push(
        "Consider if your message aligns with community guidelines",
      );
    }
  ...
  private requiresManualReview(
    ...
    if (categories.some((cat) => sensitiveCategories.includes(cat)))
      return true;
  ...
  async getModerationQueue(
    ...
    return await storage.admin.security.getModerationQueue(
      userId,
      isAdmin,
      filters,
    );
