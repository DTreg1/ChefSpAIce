...
export type FallbackAppliance = Omit<
  Appliance,
...
export type FormattedAppliance = {
  id: number;
  name: string;
  category: string;
...
export type FormattedUserAppliance = {
  id: number;
  applianceId: number;
...
function formatApplianceResponse(appliance: FallbackAppliance): FormattedAppliance {
  return {
    id: appliance.id,
    name: appliance.name,
...
export async function getAllAppliances(category?: string): Promise<FormattedAppliance[]> {
  ...
  return allAppliances.map(formatApplianceResponse);
}

export async function getCommonAppliances(): Promise<FormattedAppliance[]> {
  ...
  return commonAppliances.map(formatApplianceResponse);
}

export async function getUserAppliances(userId: string): Promise<FormattedUserAppliance[]> {
...
export async function addUserAppliance(
  userId: string,
  applianceId: number,
  notes?: string,
  brand?: string,
): Promise<FormattedUserAppliance> {
  ...
  return formatUserApplianceResponse({ ...created, appliance });
}

export async function removeUserAppliance(userId: string, applianceId: number): Promise<void> {
...
export async function bulkSyncAppliances(
  userId: string,
  applianceIds: number[],
  ...
  if (toAdd.length > 0) {
    ...
    actuallyAdded = validToAdd.length;
  }

  if (actuallyAdded > 0 || toRemove.length > 0) {
    await updateCookwareSectionTimestamp(userId);
  }
