...
const REPLACEMENTS = [
  ...
  // Database patterns
  {
    ...
    replacement: function(match, varName) {
      ...
      if (match.includes('values(') || match.includes('set(') || match.includes('insert(')) {
        return varName + ')';
      }
      return match;
    }
...
function processFile(filePath) {
  ...
  if (filePath.includes('/storage/') && filePath.includes('.storage.ts')) {
    ...
    if (modified && !content.includes('storage-helpers')) {
      const importLine = 'import { createInsertData, createUpdateData, buildMetadata } from "../../types/storage-helpers";\n';
  ...
  if (needsHelperImport && filePath.includes('/routers/') && !content.includes('request-helpers')) {
    const importLine = 'import { getAuthenticatedUserId } from "../types/request-helpers";\n';
    content = content.replace(
      /(import .* from ['"]express['"];?\n)/,
      `$1${importLine}`
    );
...
function main() {
  ...
  try {
    const result = execSync('grep -r "as any" server/ --include="*.ts" | grep -v node_modules | wc -l', {
      cwd: PROJECT_ROOT,
      encoding: 'utf8'
    });
