...
function processFile(filePath) {
  ...
  if (modified) {
    ...
    content = content.replace(/import\s+type\s+{\s*Request,\s*Response\s*}\s+from\s+"express";\s*/g, '');
...
function main() {
  ...
  const middlewareFiles = glob.sync(path.join(MIDDLEWARE_DIR, '*.middleware.ts'));
  ...
  console.log('2. Review schema mismatches in writing-assistant.router.ts and scheduling.router.ts');
