...
async function testUserEndpoints(): Promise<void> {
  ...
  // Auth endpoints
  results.push(
    await testProtectedEndpoint("GET", "/auth/status", "Auth status"),
  );
  results.push(await testProtectedEndpoint("GET", "/auth/me", "Current user"));

  // Inventory endpoints
  results.push(
    await testProtectedEndpoint("GET", "/inventory", "List inventory"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/food-items", "List food items"),
  );
  ...
  // Meal planning
  results.push(
    await testProtectedEndpoint("GET", "/meal-plans", "List meal plans"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/shopping-list", "Get shopping list"),
  );

  // Chat endpoints
  results.push(
    await testProtectedEndpoint("GET", "/chat/history", "Chat history"),
  );

  // Utility endpoints
  results.push(
    await testProtectedEndpoint("GET", "/appliances", "List appliances"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/nutrition/daily", "Daily nutrition"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/cooking-terms", "Cooking terms"),
  );
  results.push(
    await testProtectedEndpoint(
      "GET",
...
async function testAdminEndpoints(): Promise<void> {
  ...
  results.push(
    await testProtectedEndpoint("GET", "/admin/users", "Admin users list"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/admin/experiments", "A/B experiments"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/admin/cohorts", "User cohorts"),
  );
  results.push(
    await testProtectedEndpoint(
      "GET",
  ...
  results.push(
    await testProtectedEndpoint("GET", "/admin/tickets", "Support tickets"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/admin/pricing", "Pricing tiers"),
  );
  results.push(
    await testProtectedEndpoint(
      "GET",
  ...
  results.push(
    await testProtectedEndpoint("GET", "/admin/ai-metrics", "AI metrics"),
  );
}
...
async function testAIEndpoints(): Promise<void> {
  ...
  // AI Vision endpoints
  results.push(
    await testProtectedEndpoint("GET", "/ai/vision/languages", "OCR languages"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/ai/vision/stats", "Vision stats"),
  );
  ...
  results.push(
    await testProtectedEndpoint("GET", "/ai/voice/commands", "Voice commands"),
  );
  ...
  results.push(
    await testProtectedEndpoint(
      "GET",
      "/ai/recommendations/content/:id/related",
      "Recommendations",
    ),
  );
  results.push(
    await testProtectedEndpoint("GET", "/ai/insights/daily", "Daily insights"),
  );
}
...
async function testPlatformEndpoints(): Promise<void> {
  ...
  // Analytics endpoints
  results.push(
    await testProtectedEndpoint("GET", "/analytics/events", "Analytics events"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/analytics/stats", "Analytics stats"),
  );

  // Activity logs
  results.push(
    await testProtectedEndpoint("GET", "/activities", "Activity logs"),
  );

  // Notifications
  results.push(
    await testProtectedEndpoint("GET", "/notifications", "Notifications list"),
  );
  results.push(
    await testProtectedEndpoint("GET", "/notifications/tokens", "Push tokens"),
  );
  results.push(
    await testProtectedEndpoint(
      "GET",
      "/notifications/intelligent/preferences",
  ...
  // Batch operations
  results.push(
    await testProtectedEndpoint("GET", "/batch/status", "Batch status"),
  );

  // Feedback
  results.push(
    await testProtectedEndpoint("GET", "/feedback", "User feedback"),
  );
}
...
async function testSpecializedEndpoints(): Promise<void> {
  ...
  // Scheduling
  results.push(
    await testProtectedEndpoint("GET", "/scheduling/calendar", "Calendar"),
  );

  // Images
  results.push(
    await testProtectedEndpoint("GET", "/images/assets", "Image assets"),
  );
}
...
async function testErrorHandling(): Promise<void> {
  ...
  results.push(
    await testEndpoint("POST", "/ai/generation/analyze", {
  ...
  try {
    ...
    if (failed > 0) {
      ...
      results
        ...
        .forEach((r) => {
          console.log(
            `  ${r.method} ${r.endpoint}: ${r.message} (${r.statusCode || "N/A"})`,
          );
        });
    }
    ...
    results.forEach((r) => {
      ...
      console.log(
        `${statusIcon} ${r.method.padEnd(6)} ${r.endpoint.padEnd(45)} ${time.padStart(8)} ${r.statusCode || "-"}`,
      );
    });
