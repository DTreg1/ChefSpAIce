- [Introduction](https://www.barcodelookup.com/api-documentation#intro)
- [Authentication](https://www.barcodelookup.com/api-documentation#authentication)
- [Endpoints](https://www.barcodelookup.com/api-documentation#endpoints)
- [Code Examples](https://www.barcodelookup.com/api-documentation#code-examples)
- [API Queries](https://www.barcodelookup.com/api-documentation#api-queries)
- [Rate Limits](https://www.barcodelookup.com/api-documentation#rate-limits)
- [HTTP Status Codes](https://www.barcodelookup.com/api-documentation#http-status-codes)
- [Update Account](https://www.barcodelookup.com/api-documentation#update-account)

# API Documentation

The Barcode Lookup API documentation describes how to use our RESTful API service. All API responses are returned in JSON format. If you have additional questions, please email [support](mailto:support@barcodelookup.com).
...
#### GEThttps://api.barcodelookup.com/v3/products
...
| category | string | Home & Garden > Decor | search by category - based on [Google's product taxonomy](https://www.google.com/basepages/producttype/taxonomy.en-US.txt) |
...
| --- | --- | --- |
| barcode\_number | integer | 7433144698629 |
| barcode\_formats | string | UPC-A 811158031873, EAN-13 0811158031873 |
...
| age\_group | string | newborn |
| ingredients | string | Tomatoes, Onions, Celery |
| nutrition\_facts | string | Calories 85, Protein 7g |
| energy\_efficiency\_class | string | A+ (A+++ to D) |
| color | string | Purple |
| gender | string | Male |
...
| price | string | 12.99 |
| sale\_price | string | 7.49 |
| tax | array | country: US, region: US, rate: 7.00, tax\_ship: no |
| link | string | https://www.storename.com/product-link |
| item\_group\_id | string | A1BXC-GD2 |
| availability | string | in stock |
| condition | string | new |
| shipping | array | country: US, region: US, service: Ground, price: 7.29 USD |
| last\_update | string | 2022-04-17 12:09:18 |
...
#### GEThttps://api.barcodelookup.com/v3/rate-limits
...
Use double quotes on **manufacturer** and **brand** values to get an exact match. Without quotes, the results may include values with additional words or variations - a _fuzzy match_ result.
...
Set the **metadata** parameter to 'y' to retrieve the number of pages and products for any search query. You'll also retrieve the 'next\_cursor' value, which is useful for looping through the results.

`https://api.barcodelookup.com/v3/products?manufacturer=samsung&metadata=y&cursor=y&formatted=y&key=your_api_key`

### Cursor

Use the **cursor** parameter in conjunction with the **metadata** parameter to loop through all the API results. Your first query should be 'cursor=y'. Take the 'next\_cursor' value from the metadata response and set the value accordingly for 'cursor' on each consecutive query. Utilize the 'metadata' and 'cursor' parameter for any API query that returns multiple results, including all 'search' parameters and partial 'barcode' queries.
...
Your monthly API call limit is determined by your [API plan subscription](https://www.barcodelookup.com/api#sign-up).

The monthly API quota is only deducted when data is returned from a request, which will have a [HTTP 200 status response code](https://www.barcodelookup.com/api-documentation#http-status-codes).
...
You can update your API customer profile and billing information by [logging in to your API account](https://account.barcodelookup.com/).