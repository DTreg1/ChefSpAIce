 ...
 * Query Logs Table
 * 
 * Stores history of natural language queries converted to SQL.
 * Tracks query performance and provides audit trail.
 * 
 * Core Fields:
 * - id: UUID primary key
 * - userId: Foreign key to users.id (CASCADE delete)
 * - naturalQuery: Original natural language question from user
 * - generatedSql: SQL query generated by AI
 * - resultCount: Number of rows returned by query
 ...
 * - errorMessage: Error message if query failed
 ...
 * - createdAt: When query was executed
 * 
 * Business Rules:
 * - Log all query attempts for debugging and optimization
 * - Track execution time for performance monitoring
 * - Allow users to save useful queries for reuse
 * - Support query history and analytics
 * 
 * Indexes:
 * - query_logs_user_id_idx: User's query history lookup
 * - query_logs_created_at_idx: Time-based queries
 * - query_logs_is_saved_idx: Fast saved query retrieval
 * 
 * Relationships:
 * - users → queryLogs: CASCADE (delete logs when user deleted)
 */
export const queryLogs = pgTable("query_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  queryType: varchar("query_type", { length: 20 }).default('SELECT'), // SELECT, INSERT, UPDATE, DELETE
  ...
  index("query_logs_user_id_idx").on(table.userId),
