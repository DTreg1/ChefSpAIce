 ...
 * Content Embeddings Table
 * 
 * Stores OpenAI embeddings for content items to enable semantic similarity search.
 * Powers the recommendation system by converting text to high-dimensional vectors.
 * 
 * Core Fields:
 * - id: UUID primary key
 * - contentId: Unique identifier for the content (article, recipe, etc.)
 * - contentType: Type of content ('article', 'recipe', 'product', etc.)
 * - embedding: Vector representation from OpenAI (1536 dimensions for ada-002)
 * - embeddingModel: Model used to generate embedding (e.g., 'text-embedding-ada-002')
 * - embeddingDimensions: Number of dimensions in the embedding vector
 * 
 * Content Metadata:
 * - title: Content title for display
 * - description: Brief description or excerpt
 * - metadata: JSONB for additional content-specific data
 ...
 * 
 * Performance & Caching:
 * - textHash: SHA-256 hash of original text to detect changes
 * - generatedAt: When embedding was generated
 * - updatedAt: Last update timestamp
 * - isStale: Flag indicating if content has changed since embedding
 * 
 * Business Rules:
 * - One embedding per unique contentId
 * - Regenerate if textHash changes
 * - Cache embeddings to minimize API calls
 * - Support batch generation for efficiency
 * 
 * Similarity Search:
 * - Use cosine similarity for finding related content
 * - Threshold of 0.7 (70%) for relevance
 * - Return top N results ranked by similarity
 * 
 * Indexes:
 * - content_embeddings_content_id_idx: Unique constraint and fast lookup
 * - content_embeddings_content_type_idx: Filter by content type
 * - content_embeddings_updated_at_idx: Find stale embeddings
 * 
 * Example Usage:
 * ```sql
...
export const contentEmbeddings = pgTable("content_embeddings", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  contentId: varchar("content_id").notNull().unique(), // Unique identifier for the content
  contentType: varchar("content_type", { length: 50 }).notNull(), // 'article', 'recipe', 'product', etc.
  embedding: jsonb("embedding").$type<number[]>().notNull(), // Vector embedding from OpenAI
  ...
  title: text("title").notNull(), // Content title
  description: text("description"), // Content description/excerpt
  metadata: jsonb("metadata").$type<{
    category?: string;
    author?: string;
    tags?: string[];
  ...
  textHash: varchar("text_hash", { length: 64 }), // SHA-256 hash of original text
  isStale: boolean("is_stale").notNull().default(false), // Indicates if embedding needs refresh
  ...
  index("content_embeddings_updated_at_idx").on(table.updatedAt),
]);
 ...
 * Related Content Cache Table
 * 
 * Caches pre-computed related content recommendations to improve performance.
 * Avoids real-time vector similarity calculations for frequently accessed content.
 * 
 * Core Fields:
 * - id: UUID primary key
 * - contentId: Source content identifier
 * - relatedIds: Array of related content IDs with similarity scores
 * - contentType: Type of content being recommended
 * 
 * Cache Management:
 * - computedAt: When recommendations were calculated
 * - expiresAt: Cache expiration time (default: 7 days)
 * - accessCount: Number of times cache entry was accessed
 * - lastAccessedAt: Last access timestamp for LRU eviction
 * 
 * Related Content Structure (relatedIds):
 * ```json
 * [
 *   { "id": "content-123", "score": 0.95, "type": "article" },
 *   { "id": "content-456", "score": 0.87, "type": "recipe" },
 *   { "id": "content-789", "score": 0.82, "type": "article" }
 * ]
 * ```
 * 
 * Cache Strategy:
 ...
 * 
 * Business Rules:
 * - Cache top 20 related items per content
 * - Minimum similarity threshold: 0.7
 * - Refresh popular content proactively
 * - Clear cache when embeddings regenerated
 * 
 * Indexes:
 * - related_content_cache_content_id_idx: Fast lookup by content
 * - related_content_cache_expires_at_idx: Find expired entries
 * - related_content_cache_accessed_idx: LRU eviction queries
 * 
 * Relationships:
 * - References contentEmbeddings via contentId
 */
export const relatedContentCache = pgTable("related_content_cache", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  contentId: varchar("content_id").notNull().unique(), // Source content ID
  contentType: varchar("content_type", { length: 50 }).notNull(), // Type of content
  relatedIds: jsonb("related_ids").$type<Array<{
    id: string;
    score: number;
    type: string;
    title?: string;
    metadata?: any;
  }>>().notNull(), // Array of related content with scores
  threshold: real("threshold").notNull().default(0.7), // Minimum similarity threshold used
  maxResults: integer("max_results").notNull().default(20), // Maximum number of results cached
  computedAt: timestamp("computed_at").notNull().defaultNow(), // When cache was generated
  expiresAt: timestamp("expires_at").notNull().default(sql`NOW() + INTERVAL '7 days'`), // Cache expiration
  accessCount: integer("access_count").notNull().default(0), // Number of cache hits
  lastAccessedAt: timestamp("last_accessed_at").defaultNow(), // Last cache access
}, (table) => [
  index("related_content_cache_content_id_idx").on(table.contentId),
  index("related_content_cache_expires_at_idx").on(table.expiresAt),
  index("related_content_cache_accessed_idx").on(table.lastAccessedAt),
]);
