...
export const notificationHistory = pgTable(
  "notification_history",
  {
    ...
    platform: text("platform").notNull(), // 'ios', 'android', 'web'
    pushTokenId: varchar("push_token_id").references(() => pushTokens.id, {
      onDelete: "set null",
    }),
 ...
 */
export const notificationScores = pgTable(
  "notification_scores",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => [
    ...
    index("notification_scores_user_type_idx").on(
      table.userId,
      table.notificationType,
    ),
 ...
 */
export const notificationFeedback = pgTable(
  "notification_feedback",
  {
    ...
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    notificationId: varchar("notification_id").references(
  ...
  (table) => [
    ...
    uniqueIndex("notification_feedback_user_notification_idx").on(
      table.userId,
...
export const notificationFeedbackTypeSchema = z.enum([
  "useful",
  "not_useful",
  "wrong_time",
  "too_frequent",
...
export const insertPushTokenSchema = createInsertSchema(pushTokens).extend({
...
export const insertNotificationScoreSchema = createInsertSchema(
  notificationScores,
).extend({
  notificationType: notificationTypeSchema,
...
export type InsertNotificationScore = z.infer<
  typeof insertNotificationScoreSchema
>;
...
export type InsertNotificationFeedback = z.infer<
  typeof insertNotificationFeedbackSchema
>;
