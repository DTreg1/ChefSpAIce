...
export const generatedDrafts = pgTable(
  "generated_drafts",
  {
    ...
    templateId: varchar("template_id").references(() => draftTemplates.id, {
      onDelete: "set null",
    }),
 ...
 */
export const writingSuggestions = pgTable(
  "writing_suggestions",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    sessionId: varchar("session_id").references(() => writingSessions.id, {
      onDelete: "cascade",
    }),
 ...
 */
export const excerptPerformance = pgTable(
  "excerpt_performance",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    excerptId: varchar("excerpt_id").references(() => excerpts.id, {
      onDelete: "cascade",
    }),
...
export const translations = pgTable(
  "translations",
  {
    ...
    confidence: real("confidence"), // 0-1
    alternativeTranslations: jsonb("alternative_translations").$type<
      string[]
    >(),
 ...
 */
export const languagePreferences = pgTable(
  ...
  (table) => [index("language_preferences_user_id_idx").on(table.userId)],
);
...
export const excerptCategorySchema = z.enum([
  "quote",
  "fact",
  "opinion",
  "conclusion",
...
export const insertVoiceCommandSchema = createInsertSchema(
  voiceCommands,
).extend({
...
export const insertGeneratedDraftSchema = createInsertSchema(
  generatedDrafts,
).extend({
...
export const insertWritingSessionSchema = createInsertSchema(
  writingSessions,
).extend({
...
export const insertWritingSuggestionSchema = createInsertSchema(
  writingSuggestions,
).extend({
...
export type InsertWritingSuggestion = z.infer<
  typeof insertWritingSuggestionSchema
>;
export type WritingSuggestion = typeof writingSuggestions.$inferSelect;

export const insertSummarySchema = createInsertSchema(summaries).extend({
...
export const insertTranslationSchema = createInsertSchema(translations).extend({
  confidence: z.number().min(0).max(1).optional(),
...
export const insertExcerptPerformanceSchema =
  createInsertSchema(excerptPerformance);
export type InsertExcerptPerformance = z.infer<
  typeof insertExcerptPerformanceSchema
>;
export type ExcerptPerformance = typeof excerptPerformance.$inferSelect;

export const insertLanguagePreferenceSchema =
  createInsertSchema(languagePreferences);
export type InsertLanguagePreference = z.infer<
  typeof insertLanguagePreferenceSchema
>;
