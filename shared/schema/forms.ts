...
export const autoSaveDrafts = pgTable(
  "auto_save_drafts",
  {
    ...
    documentType: text("document_type").$type<
      "chat" | "recipe" | "note" | "meal_plan" | "shopping_list" | "other"
    >(),
  ...
  (table) => [
    ...
    uniqueIndex("auto_save_drafts_unique_version_idx").on(
      table.documentId,
      table.userId,
      table.version,
    ),
 ...
 */
export const savePatterns = pgTable(
  "save_patterns",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => [uniqueIndex("save_patterns_user_idx").on(table.userId)],
);
...
export const formCompletions = pgTable(
  "form_completions",
  {
    ...
    commonValues: jsonb("common_values").$type<
      Array<{
        value: string;
        count: number;
    ...
    contextRules: jsonb("context_rules").$type<
      Array<{
 ...
 * - userId: For user-specific queries
 */
export const userFormHistory = pgTable(
  "user_form_history",
  {
    id: varchar("id")
      .primaryKey()
    ...
    valuesUsed: jsonb("values_used").$type<
      Array<{
        value: string;
        count: number;
        lastUsed: string; // ISO date
    ...
    lastSequence: jsonb("last_sequence").$type<
      Array<{
        fieldName: string;
        value: string;
        timestamp: string;
  ...
  (table) => [
    uniqueIndex("user_form_history_user_field_idx").on(
      table.userId,
      table.fieldName,
    ),
 ...
 */
export const completionFeedback = pgTable(
  "completion_feedback",
  {
    id: varchar("id")
      .primaryKey()
...
export const userResolutionSchema = z.enum([
  "accepted_suggestion",
  "manual_correction",
  "ignored",
  "abandoned",
...
// Auto-Save Drafts
export const insertAutoSaveDraftSchema = createInsertSchema(
  autoSaveDrafts,
).extend({
...
// Save Patterns
export const insertSavePatternSchema = createInsertSchema(savePatterns).extend({
...
// Form Completions
export const insertFormCompletionSchema = createInsertSchema(
  formCompletions,
).extend({
...
export type InsertCompletionFeedback = z.infer<
  typeof insertCompletionFeedbackSchema
>;
