 ...
 */
export const pricingPerformance = pgTable(
  "pricing_performance",
  {
    id: varchar("id")
      .primaryKey()
  ...
  (table) => [
    ...
    index("pricing_performance_period_idx").on(
      table.periodStart,
      table.periodEnd,
    ),
  ],
...
export const insertPricingRulesSchema = createInsertSchema(pricingRules)
  ...
  .refine(
    (data) =>
      data.minPrice <= data.basePrice && data.basePrice <= data.maxPrice,
    {
...
// Price History
export const insertPriceHistorySchema = createInsertSchema(priceHistory).extend(
  {
...
export type InsertPricingPerformance = z.infer<
  typeof insertPricingPerformanceSchema
>;
