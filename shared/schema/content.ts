...
export const contentCategories = pgTable(
  ...
  (table) => [
    index("content_categories_content_idx").on(
      table.contentId,
      table.contentType,
    ),
    index("content_categories_category_id_idx").on(table.categoryId),
    uniqueIndex("content_categories_unique_idx").on(
      table.contentId,
      table.categoryId,
    ),
  ],
...
export const contentEmbeddings = pgTable(
  ...
  (table) => [
    index("content_embeddings_content_idx").on(
      table.contentId,
      table.contentType,
    ),
 ...
 */
export const relatedContentCache = pgTable(
  "related_content_cache",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
  ...
  (table) => [
    uniqueIndex("related_content_cache_content_idx").on(
      table.contentId,
      table.contentType,
    ),
...
export const insertContentTagSchema = createInsertSchema(contentTags).extend({
  contentType: contentTypeSchema,
});
...
export type InsertContentEmbedding = z.infer<
  typeof insertContentEmbeddingSchema
>;
export type ContentEmbedding = typeof contentEmbeddings.$inferSelect;

export const insertDuplicatePairSchema = createInsertSchema(
  duplicatePairs,
).extend({
  contentType: contentTypeSchema,
...
export type InsertRelatedContentCache = z.infer<
  typeof insertRelatedContentCacheSchema
>;
