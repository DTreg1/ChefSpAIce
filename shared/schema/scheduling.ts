...
export const schedulingPreferences = pgTable(
  "scheduling_preferences",
  {
    ...
    blockedTimes: jsonb("blocked_times")
      .$type<
        Array<{
          start: string;
          end: string;
 ...
 */
export const meetingSuggestions = pgTable(
  "meeting_suggestions",
  {
    id: varchar("id")
      .primaryKey()
 ...
 */
export const schedulingPatterns = pgTable(
  "scheduling_patterns",
  {
    id: varchar("id")
      .primaryKey()
    ...
    userId: varchar("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    commonMeetingTimes: jsonb("common_meeting_times")
      .$type<
        Array<{
          dayOfWeek: number;
          timeOfDay: string;
      ...
      .notNull()
      .default([]),
    meetingFrequency: jsonb("meeting_frequency")
      .$type<{
        daily: number;
        weekly: number;
        monthly: number;
...
export const meetingEvents = pgTable(
  "meeting_events",
  {
    ...
    meetingSuggestionId: varchar("meeting_suggestion_id").references(
      () => meetingSuggestions.id,
      { onDelete: "set null" },
    ),
    metadata: jsonb("metadata")
      .$type<{
...
export type InsertSchedulingPreferences = z.infer<
  typeof insertSchedulingPreferencesSchema
>;
...
export type InsertMeetingSuggestions = z.infer<
  typeof insertMeetingSuggestionsSchema
>;
export type MeetingSuggestions = typeof meetingSuggestions.$inferSelect;

// Scheduling Patterns
export const insertSchedulingPatternsSchema = createInsertSchema(
  schedulingPatterns,
).extend({
...
export type InsertSchedulingPatterns = z.infer<
  typeof insertSchedulingPatternsSchema
>;
...
// Meeting Schedules
export const meetingScheduleStatusSchema = z.enum([
  "scheduled",
  "cancelled",
  "completed",
]);
...
export type InsertMeetingSchedule = z.infer<
  typeof insertMeetingSchedulesSchema
>;
