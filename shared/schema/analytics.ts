...
export const insightFeedback = pgTable(
  "insight_feedback",
  {
    id: varchar("id")
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    insightId: varchar("insight_id")
      .notNull()
      .references(() => analyticsInsights.id, { onDelete: "cascade" }),
    userId: varchar("user_id").references(() => users.id, {
      onDelete: "set null",
    }),
...
export const predictionAccuracy = pgTable(
  "prediction_accuracy",
  {
    ...
    predictionId: varchar("prediction_id")
      .notNull()
      .references(() => userPredictions.id, { onDelete: "cascade" }),
...
export const insertUserSessionSchema = createInsertSchema(userSessions).extend({
...
export const insertWebVitalSchema = createInsertSchema(webVitals).extend({
  metric: metricSchema,
...
export const insertSearchLogSchema = createInsertSchema(searchLogs).extend({
...
export const insertAnalyticsInsightSchema = createInsertSchema(
  analyticsInsights,
).extend({
...
export type InsertAnalyticsInsight = z.infer<
  typeof insertAnalyticsInsightSchema
>;
export type AnalyticsInsight = typeof analyticsInsights.$inferSelect;

export const insertUserPredictionSchema = createInsertSchema(
  userPredictions,
).extend({
...
export const insertTrendSchema = createInsertSchema(trends).extend({
...
export const insertQueryLogSchema = createInsertSchema(queryLogs).extend({
...
export const insertPredictionAccuracySchema = createInsertSchema(
  predictionAccuracy,
).extend({
...
export type InsertPredictionAccuracy = z.infer<
  typeof insertPredictionAccuracySchema
>;
export type PredictionAccuracy = typeof predictionAccuracy.$inferSelect;

export const insertTrendAlertSchema = createInsertSchema(trendAlerts).extend({
  alertType: z.enum([
    "threshold_exceeded",
    "anomaly",
    "prediction",
