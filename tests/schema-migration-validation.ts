...
test(`Found all insert schemas (expected 102+)`, () => {
  ...
  if (allInsertSchemas.length < 102) {
    throw new Error(
      `Expected at least 102 schemas, found ${allInsertSchemas.length}`,
    );
  }
});
...
test("All insert schemas are valid Zod schemas", () => {
  ...
  console.log(
    `  Validated ${allInsertSchemas.length} schemas have Zod methods`,
  );
});
...
test("Auto-generated fields are omitted across all schemas", () => {
    ...
    for (const { field, value } of testCases) {
      ...
      if (!result.success) {
        const hasFieldError = result.error.issues.some(
          (issue) =>
            issue.path.includes(field) || issue.code === "invalid_type",
        );
  ...
  console.log(
    `  ${schemasWithOmittedFields}/${allInsertSchemas.length} schemas validate field restrictions`,
  );
});
...
test("Schemas with JSON columns properly use .extend()", () => {
  const jsonColumnSchemas = [
    {
      name: "insertNotificationPreferencesSchema",
      schema: allSchemas.insertNotificationPreferencesSchema,
    },
    {
      name: "insertSchedulingPreferencesSchema",
      schema: allSchemas.insertSchedulingPreferencesSchema,
    },
    {
      name: "insertExtractionTemplateSchema",
      schema: allSchemas.insertExtractionTemplateSchema,
    },
    {
      name: "insertPriceHistorySchema",
      schema: allSchemas.insertPriceHistorySchema,
    },
  ...
  console.log(
    `  Validated ${jsonColumnSchemas.length} schemas with JSON columns`,
  );
});
...
console.log(
  `  • ${allInsertSchemas.length} insert schemas successfully migrated`,
);
console.log(
  `  • All schemas are valid Zod schemas with .omit().extend() pattern`,
);
