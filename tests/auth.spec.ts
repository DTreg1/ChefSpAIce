...
test.describe("Authentication and Onboarding", () => {
  test("should display landing page for unauthenticated users", async ({
    ...
    await expect(
      page.getByText("Your AI-powered kitchen assistant"),
    ).toBeVisible();
  ...
  test("should show email registration form when clicking email signup", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("should show email login form when switching to login tab", async ({
    page,
  }) => {
    await page.goto("/");
